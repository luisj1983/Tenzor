 ...
 * @brief Implementation of the HRM-autograd showcase training loop.
 *
 * The training body was previously inline in autograd.cpp's main(). It was
 * extracted so tests/examples/test_hrm_example.cpp can call it with
 * epochs=50 and assert loss decreases — a regression harness against the
 * IndexSelect/Narrow negative-dim backward bug that originally surfaced
 * here (commit 6cffd0a7).
 */
...
int correct_count(const ::tenzor::Tensor& logits, const ::tenzor::Tensor& target) {
    auto lp = logits.cpu();
    auto tp = target.cpu();
...
int run_hrm_training(int epochs,
    ...
    if (verbose) showcase::print_section("Training");

    double final_loss_seen = 0.0;
    ...
    if (out_final) *out_final = final_loss_seen;
