...
TEST(OptimizerTest, SGD_MultipleParameters) {
    auto param1 = Variable(ones({2, 2}, DType::Float32), true);
    auto param2 = Variable(full({2, 2}, 2.0f, DType::Float32), true);
...
TEST(OptimizerTest, Adam_ConvergenceTest) {
    ...
    auto param = Variable(ones({4}, DType::Float32), true);
...
TEST(OptimizerTest, SGD_SimpleLinearRegression) {
    ...
    auto weight = Variable(zeros({1}, DType::Float32), true);
    auto bias = Variable(zeros({1}, DType::Float32), true);

    std::vector<Variable*> params = {&weight, &bias};
