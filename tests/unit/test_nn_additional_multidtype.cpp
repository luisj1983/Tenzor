...
class NNMultiDTypeTest : public ::testing::TestWithParam<BackendDTypeParam> {
protected:
    Device device;
    DType dtype;
    float tolerance;
    ...
    void expectTensorNear(const Tensor& a, const Tensor& b, float tol = -1.0f) {
        ...
        if (dtype == DType::Float32) {
            ...
            for (int64_t i = 0; i < a.numel(); ++i) {
                EXPECT_NEAR(a_data[i], b_data[i], tol) << "Failed at index " << i;
            }
...
INSTANTIATE_TEST_SUITE_P(
    ...
    NNMultiDTypeTest,
    ::testing::ValuesIn(GenerateNNBackendDTypeCombinations()),
