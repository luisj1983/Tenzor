...
class VisionComponentsMultiDTypeTest : public MultiBackendDTypeTest {
protected:
    // Cast a tensor to the parameterized test dtype (no-op for Float32).
    Tensor toTestDType(const Tensor& t) {
        return (dtype() == DType::Float32) ? t : t.to(dtype());
    }
    // Variable-aware shape check that also asserts the tensor dtype.
