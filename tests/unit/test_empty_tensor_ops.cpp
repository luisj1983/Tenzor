...
class EmptyTensorOpsTest : public ::tenzor::testing::BackendTest {
protected:
    void SetUp() override {
...
TEST_P(EmptyTensorOpsTest, ZerosEmptyShapes) {
    // {0} - 1D empty
...
TEST_P(EmptyTensorOpsTest, OnesEmptyShapes) {
...
TEST_P(EmptyTensorOpsTest, FullEmptyShapes) {
...
TEST_P(EmptyTensorOpsTest, RandnEmptyShapes) {
...
TEST_P(EmptyTensorOpsTest, CreationPreservesDtype) {
...
TEST_P(EmptyTensorOpsTest, AddEmptyTensors) {
    // Same empty shape
...
TEST_P(EmptyTensorOpsTest, AddEmptyWithBroadcast) {
    auto a = zeros({0, 4}, DType::Float32, device);
...
TEST_P(EmptyTensorOpsTest, MulEmptyTensors) {
...
TEST_P(EmptyTensorOpsTest, SubEmptyTensors) {
...
TEST_P(EmptyTensorOpsTest, MatmulEmptyInnerDim) {
...
TEST_P(EmptyTensorOpsTest, MatmulEmptyOuterDim) {
    // (0, 4) x (4, 3) -> (0, 3)
    auto a = zeros({0, 4}, DType::Float32, device);
...
TEST_P(EmptyTensorOpsTest, SumEmptyGlobal) {
...
TEST_P(EmptyTensorOpsTest, SumEmptyAlongEmptyDim) {
...
TEST_P(EmptyTensorOpsTest, SumEmptyAlongNonEmptyDim) {
...
TEST_P(EmptyTensorOpsTest, SumEmpty1D) {
...
TEST_P(EmptyTensorOpsTest, MeanEmpty1DReturnsNaN) {
    ...
    EXPECT_NO_FATAL_FAILURE({
        try {
            ...
            if (result.numel() == 1) {
                float val = result.cpu().item<float>();
...
TEST_P(EmptyTensorOpsTest, MaxEmpty1D) {
...
TEST_P(EmptyTensorOpsTest, MinEmpty1D) {
...
TEST_P(EmptyTensorOpsTest, SumEmptyKeepDim) {
...
TEST_P(EmptyTensorOpsTest, ReshapeEmptyPreservesNumel) {
...
TEST_P(EmptyTensorOpsTest, ReshapeEmptyToFlat) {
    auto t = zeros({0, 5}, DType::Float32, device);
    auto r = reshape(t, {0});
    EXPECT_EQ(r.numel(), 0);
    EXPECT_EQ(r.ndim(), 1);
...
TEST_P(EmptyTensorOpsTest, ReshapeEmptySwapDims) {
...
TEST_P(EmptyTensorOpsTest, TransposeEmpty2D) {
    auto t = zeros({0, 5}, DType::Float32, device);
...
TEST_P(EmptyTensorOpsTest, TransposeEmptyMiddleDim) {
...
TEST_P(EmptyTensorOpsTest, SqueezeEmptyNoop) {
...
TEST_P(EmptyTensorOpsTest, SqueezeEmptyRemovesUnitDim) {
...
TEST_P(EmptyTensorOpsTest, UnsqueezeEmpty) {
...
TEST_P(EmptyTensorOpsTest, PermuteEmpty3D) {
...
TEST_P(EmptyTensorOpsTest, CloneEmpty) {
    auto t = zeros({0, 3}, DType::Float32, device);
    auto c = t.clone();
    EXPECT_EQ(c.numel(), 0);
    EXPECT_EQ(c.shape()[0], 0);
...
TEST_P(EmptyTensorOpsTest, ToDtypeEmpty) {
    auto t = zeros({0, 3}, DType::Float32, device);
...
TEST_P(EmptyTensorOpsTest, ChainedOpsOnEmpty) {
    ...
    auto s = sum(d);
    EXPECT_EQ(s.numel(), 1);
    EXPECT_FLOAT_EQ(s.cpu().item<float>(), 0.0f);
}

TEST_P(EmptyTensorOpsTest, ReshapeTransposeChained) {
    auto t = zeros({0, 6}, DType::Float32, device);
    auto r = reshape(t, {0, 2, 3});
...
TEST_P(EmptyTensorOpsTest, AllZeroDims) {
    // {0, 0} tensor
...
TEST_P(EmptyTensorOpsTest, HigherRankEmpty) {
...
INSTANTIATE_BACKEND_TESTS(EmptyTensorOpsTest);
