...
class DTypeConversionChainTest : public ::tenzor::testing::BackendTest {
protected:
    void SetUp() override {
...
TEST_P(DTypeConversionChainTest, F32_F16_F32_Exact_Representable) {
    ...
    auto* rt = roundtrip.cpu().data<float>();
...
TEST_P(DTypeConversionChainTest, F32_F16_F32_Approximate_Values) {
...
TEST_P(DTypeConversionChainTest, F32_F16_F32_SmallValues) {
...
TEST_P(DTypeConversionChainTest, F32_F16_F32_LargeValues) {
...
TEST_P(DTypeConversionChainTest, F32_BF16_F32_Exact_Representable) {
    auto data = zeros({5}, DType::Float32, Device::cpu());
    auto* ptr = data.data<float>();
    ptr[0] = 0.0f;
    ptr[1] = 1.0f;
...
TEST_P(DTypeConversionChainTest, F32_BF16_F32_Approximate_Values) {
...
TEST_P(DTypeConversionChainTest, F32_BF16_F32_LargeRange) {
...
TEST_P(DTypeConversionChainTest, NaN_F32_F16_F32) {
...
TEST_P(DTypeConversionChainTest, NaN_F32_BF16_F32) {
    auto data = zeros({2}, DType::Float32, Device::cpu());
    auto* ptr = data.data<float>();
    ptr[0] = std::numeric_limits<float>::quiet_NaN();
    ptr[1] = 2.0f;
...
TEST_P(DTypeConversionChainTest, Inf_F32_F16_F32) {
    auto data = zeros({3}, DType::Float32, Device::cpu());
    auto* ptr = data.data<float>();
...
TEST_P(DTypeConversionChainTest, Inf_F32_BF16_F32) {
    auto data = zeros({3}, DType::Float32, Device::cpu());
    auto* ptr = data.data<float>();
    ptr[0] = std::numeric_limits<float>::infinity();
    ptr[1] = -std::numeric_limits<float>::infinity();
...
TEST_P(DTypeConversionChainTest, MaxFloat32_to_F16_Overflow) {
...
TEST_P(DTypeConversionChainTest, MaxFloat32_to_BF16_Preserves_Range) {
...
TEST_P(DTypeConversionChainTest, NegativeZero_F32_F16_F32) {
    auto data = zeros({2}, DType::Float32, Device::cpu());
    auto* ptr = data.data<float>();
    ptr[0] = -0.0f;
    ptr[1] = 0.0f;
...
TEST_P(DTypeConversionChainTest, NegativeZero_F32_BF16_F32) {
    auto data = zeros({2}, DType::Float32, Device::cpu());
    auto* ptr = data.data<float>();
    ptr[0] = -0.0f;
    ptr[1] = 0.0f;
...
TEST_P(DTypeConversionChainTest, F32_F16_BF16_F32) {
...
TEST_P(DTypeConversionChainTest, F32_F64_F32) {
    ...
    EXPECT_FLOAT_EQ(rt[0], 0.1f);
    EXPECT_FLOAT_EQ(rt[1], 3.14159f);
    EXPECT_FLOAT_EQ(rt[2], 1e20f);
    EXPECT_FLOAT_EQ(rt[3], -42.5f);
}
...
TEST_P(DTypeConversionChainTest, BatchConversionConsistency) {
    ...
    auto data_dev = data.to(device);

    auto f16 = data_dev.to(DType::Float16);
    auto roundtrip = f16.to(DType::Float32);
    auto* rt = roundtrip.cpu().data<float>();
    ...
    for (int i = 0; i < n; ++i) {
        ...
        auto single_rt = single.to(DType::Float16).to(DType::Float32).cpu();
        EXPECT_FLOAT_EQ(rt[i], single_rt.data<float>()[0])
            << "Batch and single conversion differ at index " << i;
    }
}

INSTANTIATE_BACKEND_TESTS(DTypeConversionChainTest);
