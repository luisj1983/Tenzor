...
class Fp8QuantDequantGradTest : public ::tenzor::testing::BackendTest {
protected:
    void SetUp() override {
...
TEST_P(Fp8QuantDequantGradTest, GradFlowsThroughInRangeValues) {
    ...
    Tensor data_cpu({4}, DType::Float32, Device::cpu());
    auto* p = data_cpu.data<float>();
    p[0] = 1.0f; p[1] = -2.0f; p[2] = 3.0f; p[3] = -4.0f;
    Tensor data = data_cpu.to(device);
...
TEST_P(Fp8QuantDequantGradTest, GradZeroedOutsideRangeFixedScale) {
...
TEST_P(Fp8QuantDequantGradTest, ForwardPreservesShapeAndSign) {
    ...
    // entries).
    Tensor data_cpu({3}, DType::Float32, Device::cpu());
    auto* p = data_cpu.data<float>();
...
INSTANTIATE_BACKEND_TESTS(Fp8QuantDequantGradTest);
