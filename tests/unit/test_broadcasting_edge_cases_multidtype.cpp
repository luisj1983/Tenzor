/**
 * @file test_broadcasting_edge_cases_multidtype.cpp
 * @brief Multi-backend tests for broadcasting edge cases
 *
 * Converted from test_broadcasting_edge_cases.cpp to run across all backends.
 *
 * Covers:
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, Broadcast_1_1_1_N_Plus_N) {
    // (1, 1, 1, 4) + (4,) should broadcast to (1, 1, 1, 4)
    auto a_cpu = ones({1, 1, 1, 4}, DType::Float32, Device::cpu());
    auto* a_data = a_cpu.data<float>();
    a_data[0] = 1.0f; a_data[1] = 2.0f; a_data[2] = 3.0f; a_data[3] = 4.0f;

    auto b_cpu = ones({4}, DType::Float32, Device::cpu());
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, Broadcast_N_Plus_1_1_1_N) {
    ...
    auto b_cpu = ones({1, 1, 1, 4}, DType::Float32, Device::cpu());
    auto* b_data = b_cpu.data<float>();
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, ScalarAddToMatrix) {
    // (1,) + (2, 3) should broadcast to (2, 3)
    auto scalar_cpu = ones({1}, DType::Float32, Device::cpu());
    scalar_cpu.data<float>()[0] = 5.0f;

    auto matrix_cpu = zeros({2, 3}, DType::Float32, Device::cpu());
    auto* m_data = matrix_cpu.data<float>();
    for (int i = 0; i < 6; ++i) {
        m_data[i] = static_cast<float>(i + 1);
    }

    auto scalar = scalar_cpu.to(device);
    auto matrix = matrix_cpu.to(device);
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, ScalarMulToMatrix) {
    auto scalar_cpu = ones({1}, DType::Float32, Device::cpu());
    scalar_cpu.data<float>()[0] = 3.0f;

    auto matrix_cpu = ones({2, 4}, DType::Float32, Device::cpu());
    auto* m_data = matrix_cpu.data<float>();
    for (int i = 0; i < 8; ++i) {
        m_data[i] = static_cast<float>(i + 1);
    }
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, ScalarAddWithOperator) {
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, Broadcast_N1_Plus_1M) {
    // (3, 1) + (1, 4) should broadcast to (3, 4)
    auto a_cpu = zeros({3, 1}, DType::Float32, Device::cpu());
    auto* a_data = a_cpu.data<float>();
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, Broadcast_Batch_Plus_NonBatch) {
    // (2, 3, 4) + (3, 4) -> (2, 3, 4)
    auto a = ones({2, 3, 4}, DType::Float32, device);

    auto b_cpu = ones({3, 4}, DType::Float32, Device::cpu());
    auto* b_data = b_cpu.data<float>();
    for (int i = 0; i < 12; ++i) {
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, BackwardGradientReductionBroadcastAdd) {
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, BackwardGradientReductionBroadcastMul) {
    // x: (2, 3), s: (1,) scalar, y = x * s, loss = sum(y)
    // dL/ds should be sum of x
    auto x_cpu = ones({2, 3}, DType::Float32, Device::cpu());
    auto* xp = x_cpu.data<float>();
    for (int i = 0; i < 6; ++i) {
        xp[i] = static_cast<float>(i + 1);  // [1,2,3,4,5,6]
    }
    auto x_tensor = x_cpu.to(device);

    auto s_tensor = full({1}, 2.0f, DType::Float32, device);

    Variable x(x_tensor, true);
    Variable s(s_tensor, true);
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, Broadcast_1x1_Plus_MxN) {
    // (1, 1) + (3, 4) -> (3, 4)
...
TEST_P(BroadcastingEdgeCaseMultiBackendTest, Broadcast_1_Plus_1_1_1) {
    // (1,) + (1, 1, 1) -> (1, 1, 1)
...
INSTANTIATE_BACKEND_TESTS(BroadcastingEdgeCaseMultiBackendTest);
