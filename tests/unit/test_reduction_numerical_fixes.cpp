...
class ReductionNumericalFixesEnv : public ::testing::Environment {
public:
    void SetUp() override { tenzor::initialize(); }
};

static ::testing::Environment* const g_env =
    ::testing::AddGlobalTestEnvironment(new ReductionNumericalFixesEnv);
...
TEST(ReductionNumericalFixes, NormFloat64FullPrecision) {
    ...
    EXPECT_NEAR(n0.item<double>(), expected, 1e-13)
    ...
    auto m = tz::zeros({1, 4}, tz::DType::Float64);
    double* mp = m.data<double>();
    for (int i = 0; i < 4; ++i) mp[i] = p[i];

    auto n1 = tz::norm(m, 2.0f, /*dim=*/1, /*keepdim=*/false);
    EXPECT_NEAR(n1.item<double>(), expected, 1e-13)
        << "Per-dim F64 norm lost precision (round-trip through F32?): "
        << "got " << n1.item<double>() << " expected " << expected;
}
...
static float result_as_float(const tz::Tensor& t) {
    return t.to(tz::DType::Float32).item<float>();
}

// F16 max (global reduction)
TEST(ReductionNumericalFixes, MaxNanPropagationFloat16) {
    auto t = tz::full({16}, 1.0, tz::DType::Float16);
    // Inject a quiet-NaN at index 4; F16 quiet-NaN = 0x7E00.
    set_f16_bits(t, 4, 0x7E00u);
    auto m = tz::max(t);
...
TEST(ReductionNumericalFixes, MaxNanPropagationBFloat16) {
    auto t = tz::full({16}, 1.0, tz::DType::BFloat16);
    // BF16 quiet-NaN = 0x7FC0 (or any 0x7F80..0x7FFF with mantissa != 0)
    set_f16_bits(t, 4, 0x7FC0u);
    auto m = tz::max(t);
    float val = result_as_float(m);
    EXPECT_TRUE(std::isnan(val))
...
TEST(ReductionNumericalFixes, MaxNanPropagationFloat32) {
    auto t = tz::ones({16}, tz::DType::Float32);
    t.data<float>()[4] = std::numeric_limits<float>::quiet_NaN();
    auto m = tz::max(t);
    EXPECT_TRUE(std::isnan(m.item<float>()))
        << "F32 max: expected NaN but got " << m.item<float>();
}

// F16 min (global reduction)
TEST(ReductionNumericalFixes, MinNanPropagationFloat16) {
    auto t = tz::full({16}, 1.0, tz::DType::Float16);
    set_f16_bits(t, 7, 0x7E00u);
    auto m = tz::min(t);
    float val = result_as_float(m);
    EXPECT_TRUE(std::isnan(val))
...
TEST(ReductionNumericalFixes, MinNanPropagationBFloat16) {
    auto t = tz::full({16}, 1.0, tz::DType::BFloat16);
    set_f16_bits(t, 7, 0x7FC0u);
    auto m = tz::min(t);
    float val = result_as_float(m);
    EXPECT_TRUE(std::isnan(val))
...
TEST(ReductionNumericalFixes, MinNanPropagationFloat32) {
    auto t = tz::ones({16}, tz::DType::Float32);
    t.data<float>()[4] = std::numeric_limits<float>::quiet_NaN();
    auto m = tz::min(t);
    EXPECT_TRUE(std::isnan(m.item<float>()))
        << "F32 min: expected NaN but got " << m.item<float>();
}

// ── Task 3.3: Float64 Kahan compensation ─────────────────────────────────────

TEST(ReductionNumericalFixes, SumFloat64KahanCompensation) {
    ...
    auto t = tz::full({N}, 1e-9, tz::DType::Float64);
    auto s = tz::sum(t);
    double actual   = s.item<double>();
