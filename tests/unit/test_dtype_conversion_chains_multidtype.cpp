/**
 * @file test_dtype_conversion_chains_multidtype.cpp
 * @brief Multi-backend, multi-dtype tests for dtype conversion chains and round-trip accuracy
 *
 * Covers:
...
class DTypeConversionChainMultiBackendTest : public MultiBackendDTypeTest {};

// ============================================================================
// 1. F32 -> F16 -> F32 Round-Trips
// ============================================================================

TEST_P(DTypeConversionChainMultiBackendTest, F32_F16_F32_Exact_Representable) {
    // Values that are exactly representable in Float16
    auto data = zeros({6}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
    ptr[0] = 0.0f;
    ptr[1] = 1.0f;
    ptr[2] = -1.0f;
    ptr[3] = 0.5f;
    ...
    auto roundtrip = f16.to(DType::Float32).to(Device::cpu());
...
TEST_P(DTypeConversionChainMultiBackendTest, F32_F16_F32_Approximate_Values) {
    // Values that lose some precision in Float16
    auto data = zeros({5}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
    ptr[0] = 0.1f;
    ptr[1] = 0.3f;
...
TEST_P(DTypeConversionChainMultiBackendTest, F32_F16_F32_SmallValues) {
    // Very small values that might lose precision or become zero
    auto data = zeros({4}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
    ptr[0] = 1e-4f;
    ptr[1] = 1e-5f;
...
TEST_P(DTypeConversionChainMultiBackendTest, F32_F16_F32_LargeValues) {
    // Float16 max is ~65504
    auto data = zeros({3}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
...
TEST_P(DTypeConversionChainMultiBackendTest, F32_BF16_F32_Exact_Representable) {
    auto data = zeros({5}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
    ptr[0] = 0.0f;
    ...
    auto roundtrip = bf16.to(DType::Float32).to(Device::cpu());
    auto* rt = roundtrip.data<float>();
...
TEST_P(DTypeConversionChainMultiBackendTest, F32_BF16_F32_Approximate_Values) {
...
TEST_P(DTypeConversionChainMultiBackendTest, F32_BF16_F32_LargeRange) {
...
TEST_P(DTypeConversionChainMultiBackendTest, NaN_F32_F16_F32) {
    auto data = zeros({2}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
...
TEST_P(DTypeConversionChainMultiBackendTest, NaN_F32_BF16_F32) {
    auto data = zeros({2}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
    ptr[0] = std::numeric_limits<float>::quiet_NaN();
...
TEST_P(DTypeConversionChainMultiBackendTest, Inf_F32_F16_F32) {
    auto data = zeros({3}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
...
TEST_P(DTypeConversionChainMultiBackendTest, Inf_F32_BF16_F32) {
    auto data = zeros({3}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
    ptr[0] = std::numeric_limits<float>::infinity();
...
TEST_P(DTypeConversionChainMultiBackendTest, MaxFloat32_to_F16_Overflow) {
...
TEST_P(DTypeConversionChainMultiBackendTest, MaxFloat32_to_BF16_Preserves_Range) {
...
TEST_P(DTypeConversionChainMultiBackendTest, NegativeZero_F32_F16_F32) {
    auto data = zeros({2}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
    ptr[0] = -0.0f;
...
TEST_P(DTypeConversionChainMultiBackendTest, NegativeZero_F32_BF16_F32) {
    auto data = zeros({2}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
    ptr[0] = -0.0f;
...
TEST_P(DTypeConversionChainMultiBackendTest, F32_F16_BF16_F32) {
...
TEST_P(DTypeConversionChainMultiBackendTest, F32_F64_F32) {
    ...
    auto roundtrip = f64.to(DType::Float32).to(Device::cpu());
    auto* rt = roundtrip.data<float>();
...
TEST_P(DTypeConversionChainMultiBackendTest, BatchConversionConsistency) {
    // Converting a batch of values should give the same results as converting individually
    int n = 16;
    auto data = zeros({n}, DType::Float32, device());
    auto data_cpu = data.to(Device::cpu());
    auto* ptr = data_cpu.data<float>();
    for (int i = 0; i < n; ++i) {
    ...
    for (int i = 0; i < n; ++i) {
        auto single = zeros({1}, DType::Float32, device());
        auto single_cpu = single.to(Device::cpu());
        single_cpu.data<float>()[0] = ptr[i];
        single = single_cpu.to(device());
        auto single_rt = single.to(DType::Float16).to(DType::Float32).to(Device::cpu());
        EXPECT_FLOAT_EQ(rt[i], single_rt.data<float>()[0])
            << "Batch and single conversion differ at index " << i;
    }
}

INSTANTIATE_MULTI_BACKEND_DTYPE_TESTS(DTypeConversionChainMultiBackendTest);
