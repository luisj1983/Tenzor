...
class HvpVhpTest : public ::testing::Test {};

// ============================================================================
// HVP tests
// ============================================================================

TEST_F(HvpVhpTest, HvpQuadraticIdentityHessian) {
    // f(x) = 0.5 * x^T * I * x = 0.5 * sum(x^2)
    // Hessian = I (identity)
    // hvp(f, x, v) = I @ v = v
    auto x = Variable(ones({4}, DType::Float64), true);
    float v_data[] = {1.0f, 2.0f, 3.0f, 4.0f};
    auto v = from_data(v_data, {4}).to(DType::Float64);
...
TEST_F(HvpVhpTest, HvpQuadraticScaledHessian) {
    // f(x) = 0.5 * (2*x0^2 + 4*x1^2) = x0^2 + 2*x1^2
    // Hessian = diag(2, 4)
    // hvp(f, x, v) = [2*v0, 4*v1]
    float x_data[] = {1.0f, 1.0f};
    auto x = Variable(from_data(x_data, {2}).to(DType::Float64), true);

    float v_data[] = {3.0f, 5.0f};
    auto v = from_data(v_data, {2}).to(DType::Float64);
    ...
    auto func = [](const Variable& input) -> Variable {
        ...
        float w[] = {1.0f, 2.0f};
        auto weights = Variable(from_data(w, {2}).to(DType::Float64), false);
...
TEST_F(HvpVhpTest, VhpQuadraticIdentityHessian) {
...
TEST_F(HvpVhpTest, VhpQuadraticScaledHessian) {
...
TEST_F(HvpVhpTest, HvpAndVhpAgreeForSymmetricHessian) {
    // For symmetric H, hvp(f,x,v) and vhp(f,x,v) should give the same result
    // f(x) = 0.5 * sum(x^2), H = I (symmetric)
    auto x = Variable(ones({3}, DType::Float64) * 2.0, true);
    float v_data[] = {1.0f, -1.0f, 2.0f};
    auto v = from_data(v_data, {3}).to(DType::Float64);
