...
class Int8OverflowEnv : public ::testing::Environment {
public:
    void SetUp() override { tenzor::initialize(); }
};

static ::testing::Environment* const g_env =
    ::testing::AddGlobalTestEnvironment(new Int8OverflowEnv);

TEST(Int8OverflowSemantics, AddWraps) {
    auto a = tz::full({16}, 100.0, tz::DType::Int8);
    auto b = tz::full({16}, 100.0, tz::DType::Int8);
    auto c = a + b;
    auto p = c.cpu().to(tz::DType::Int32).data<int32_t>();
    for (int i = 0; i < 16; ++i) EXPECT_EQ(p[i], -56) << "int8 add not wrapping at i=" << i;
}

TEST(Int8OverflowSemantics, MulWraps) {
    auto a = tz::full({16}, 100.0, tz::DType::Int8);
    auto b = tz::full({16},   2.0, tz::DType::Int8);
    auto c = a * b;  // 100 * 2 = 200 -> wrap to -56
    auto p = c.cpu().to(tz::DType::Int32).data<int32_t>();
    for (int i = 0; i < 16; ++i) EXPECT_EQ(p[i], -56) << "int8 mul not wrapping at i=" << i;
}

TEST(Int8OverflowSemantics, SubWraps) {
    auto a = tz::full({16}, 100.0, tz::DType::Int8);
    auto b = tz::full({16}, -100.0, tz::DType::Int8);
...
TEST(UInt8OverflowSemantics, AddWraps) {
    auto a = tz::full({16}, 200.0, tz::DType::UInt8);
    auto b = tz::full({16}, 200.0, tz::DType::UInt8);
    auto c = a + b;  // 200+200 = 400 -> wrap to 144 (400 mod 256 = 144)
    auto p = c.cpu().to(tz::DType::Int32).data<int32_t>();
    for (int i = 0; i < 16; ++i) EXPECT_EQ(p[i], 144);
}

TEST(UInt8OverflowSemantics, MulWraps) {
    // 200 * 2 = 400 -> wrap to 144
    auto a = tz::full({16}, 200.0, tz::DType::UInt8);
    auto b = tz::full({16},   2.0, tz::DType::UInt8);
    auto c = a * b;
    auto p = c.cpu().to(tz::DType::Int32).data<int32_t>();
...
TEST(Int8OverflowSemantics, MulWrapsAVX2Path) {
    auto a = tz::full({64}, 100.0, tz::DType::Int8);
    auto b = tz::full({64},   2.0, tz::DType::Int8);
    auto c = a * b;
    auto p = c.cpu().to(tz::DType::Int32).data<int32_t>();
    for (int i = 0; i < 64; ++i) EXPECT_EQ(p[i], -56) << "int8 mul SIMD path not wrapping at i=" << i;
}

TEST(Int8OverflowSemantics, AddWrapsAVX2Path) {
    auto a = tz::full({64}, 100.0, tz::DType::Int8);
    auto b = tz::full({64}, 100.0, tz::DType::Int8);
    auto c = a + b;
    auto p = c.cpu().to(tz::DType::Int32).data<int32_t>();
