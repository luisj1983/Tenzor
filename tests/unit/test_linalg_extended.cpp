...
class LinalgExtendedTest : public ::testing::Test {};

// ============================================================================
// vector_norm tests
// ============================================================================

TEST_F(LinalgExtendedTest, VectorNormL1) {
    // L1 norm of [1, -2, 3] = |1| + |-2| + |3| = 6
    float data[] = {1.0f, -2.0f, 3.0f};
    auto x = from_data(data, {3});

    auto result = vector_norm(x, 1.0);
    EXPECT_NEAR(result.data<float>()[0], 6.0f, 1e-5);
}

TEST_F(LinalgExtendedTest, VectorNormL2) {
    // L2 norm of [3, 4] = sqrt(9+16) = 5
    float data[] = {3.0f, 4.0f};
    auto x = from_data(data, {2});

    auto result = vector_norm(x, 2.0);
    EXPECT_NEAR(result.data<float>()[0], 5.0f, 1e-5);
}

TEST_F(LinalgExtendedTest, VectorNormL2Default) {
    // Default is L2
    float data[] = {3.0f, 4.0f};
    auto x = from_data(data, {2});
...
TEST_F(LinalgExtendedTest, VectorNormLinf) {
...
TEST_F(LinalgExtendedTest, MatrixNormFrobenius) {
    // Frobenius norm of [[1,2],[3,4]] = sqrt(1+4+9+16) = sqrt(30)
    float data[] = {1.0f, 2.0f, 3.0f, 4.0f};
    auto A = from_data(data, {2, 2}).to(DType::Float64);
    ...
    EXPECT_NEAR(result.data<double>()[0], std::sqrt(30.0), 1e-5);
}

TEST_F(LinalgExtendedTest, MatrixNormSpectral) {
    // Spectral norm (largest singular value) of [[1,0],[0,2]] = 2
    float data[] = {1.0f, 0.0f, 0.0f, 2.0f};
    auto A = from_data(data, {2, 2}).to(DType::Float64);

    auto result = matrix_norm(A, 2.0);
    EXPECT_NEAR(result.data<double>()[0], 2.0, 1e-4);
}
...
TEST_F(LinalgExtendedTest, HouseholderProductReproducesQR) {
    // Compute QR of a matrix, then use householder_product to reconstruct Q
    // and verify Q @ R = A
    float a_data[] = {1.0f, 2.0f, 3.0f, 4.0f};
    auto A = from_data(a_data, {2, 2}).to(DType::Float64);

    auto [Q, R] = qr(A);

    // Verify Q is orthogonal: Q^T @ Q should be identity
    // We can check this by verifying Q @ R approximates A
    auto reconstructed = Q;  // Q from qr already is the full Q

    // Check Q @ R = A (within tolerance)
    auto QR_data = Q.data<double>();
    auto R_data = R.data<double>();
...
TEST_F(LinalgExtendedTest, LdlSolveSymmetricSystem) {
...
TEST_F(LinalgExtendedTest, LdlSolveLargerSystem) {
    ...
    auto A = from_data(a_data, {3, 3}).to(DType::Float64);
    auto b = from_data(b_data, {3, 1}).to(DType::Float64);
...
TEST_F(LinalgExtendedTest, LdlFactorConsistency) {
    ...
    auto* d1 = x_direct.data<double>();
    auto* d2 = x_ldl.data<double>();
