...
class YOLOTest : public ::tenzor::testing::BackendTest {
protected:
    void SetUp() override {
        BackendTest::SetUp();
...
TEST_P(YOLOTest, YOLOv3ForwardShape) {
    auto model = yolov3(80, false);
    model->to(device);
    Variable images(Tensor({2, 3, 416, 416}, DType::Float32, device), true);
...
TEST_P(YOLOTest, YOLOv3GradientFlow) {
    auto model = yolov3(80, false);
    model->train();
    model->to(device);

    Variable images(Tensor({1, 3, 416, 416}, DType::Float32, device), true);
    auto output = model->forward(images);
...
TEST_P(YOLOTest, YOLOv3TinyForwardShape) {
...
TEST_P(YOLOTest, YOLOv5SmallForwardShape) {
    auto model = yolov5s(80, false);
    model->to(device);
    Variable images(Tensor({2, 3, 640, 640}, DType::Float32, device), true);
    auto output = model->forward(images);
...
TEST_P(YOLOTest, YOLOv5MediumForwardShape) {
    auto model = yolov5m(80, false);
    model->to(device);
    Variable images(Tensor({1, 3, 640, 640}, DType::Float32, device), true);
    auto output = model->forward(images);
...
TEST_P(YOLOTest, YOLOv5LargeForwardShape) {
...
TEST_P(YOLOTest, YOLOv5XLargeForwardShape) {
...
TEST_P(YOLOTest, YOLOv5GradientFlow) {
    auto model = yolov5s(80, false);
    model->train();
    model->to(device);
...
TEST_P(YOLOTest, YOLODifferentImageSizes) {
    auto model = yolov3(80, false);
    model->to(device);

    // YOLO v3 supports 416, 512, 608
    Variable images_416(Tensor({1, 3, 416, 416}, DType::Float32, device), true);
    auto output_416 = model->forward(images_416);
    EXPECT_TRUE(output_416.tensor().numel() > 0);

    Variable images_608(Tensor({1, 3, 608, 608}, DType::Float32, device), true);
...
TEST_P(YOLOTest, RefitAnchorsKmeansReplacesCocoDefaults_G17) {
...
TEST_P(YOLOTest, RefitAnchorsKmeansThrowsOnTooFewBoxes_G17) {
...
INSTANTIATE_BACKEND_TESTS(YOLOTest);
