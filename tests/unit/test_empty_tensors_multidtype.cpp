/**
 * @file test_empty_tensors_multidtype.cpp
 * @brief Multi-backend tests for empty tensor edge cases (zero-element tensors)
 *
 * Converted from test_empty_tensors.cpp to run across all backends.
 *
 * Covers:
...
TEST_P(EmptyTensorMultiBackendTest, ZerosWithZeroRows) {
...
TEST_P(EmptyTensorMultiBackendTest, OnesWithZeroCols) {
...
TEST_P(EmptyTensorMultiBackendTest, ZerosWithAllZeroDims) {
...
TEST_P(EmptyTensorMultiBackendTest, ZerosWithZeroMiddleDim) {
...
TEST_P(EmptyTensorMultiBackendTest, FullWithZeroDim) {
...
TEST_P(EmptyTensorMultiBackendTest, DTypePreserved) {
...
TEST_P(EmptyTensorMultiBackendTest, DevicePreserved) {
    auto t = zeros({0, 5}, DType::Float32, device);
...
TEST_P(EmptyTensorMultiBackendTest, SumOfEmptyTensor) {
    auto t = zeros({0, 5}, DType::Float32, device);
    auto result = sum(t);
...
TEST_P(EmptyTensorMultiBackendTest, SumAlongNonEmptyDim) {
...
TEST_P(EmptyTensorMultiBackendTest, SumAlongEmptyDim) {
...
TEST_P(EmptyTensorMultiBackendTest, ReshapeEmptyToEmpty) {
    auto t = zeros({0, 5}, DType::Float32, device);
...
TEST_P(EmptyTensorMultiBackendTest, ReshapeEmptyToHigherRank) {
...
TEST_P(EmptyTensorMultiBackendTest, ReshapeEmptyFlatten) {
    auto t = zeros({0, 5}, DType::Float32, device);
    auto r = t.reshape({0});
    EXPECT_EQ(r.numel(), 0);
    EXPECT_EQ(r.ndim(), 1);
...
TEST_P(EmptyTensorMultiBackendTest, NdimFor1DEmpty) {
    auto t = zeros({0}, DType::Float32, device);
    EXPECT_EQ(t.ndim(), 1);
    EXPECT_EQ(t.numel(), 0);
}

TEST_P(EmptyTensorMultiBackendTest, NdimFor3DEmpty) {
    auto t = zeros({0, 3, 4}, DType::Float32, device);
    EXPECT_EQ(t.ndim(), 3);
    EXPECT_EQ(t.numel(), 0);
}

TEST_P(EmptyTensorMultiBackendTest, NdimFor4DEmpty) {
...
TEST_P(EmptyTensorMultiBackendTest, AddEmptyWithEmpty) {
...
TEST_P(EmptyTensorMultiBackendTest, EmptyFloat64) {
...
TEST_P(EmptyTensorMultiBackendTest, EmptyInt32) {
...
INSTANTIATE_BACKEND_TESTS(EmptyTensorMultiBackendTest);
