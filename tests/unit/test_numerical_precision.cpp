...
class NumericalPrecisionTest : public ::tenzor::testing::BackendTest {
protected:
    void SetUp() override {
...
TEST_P(NumericalPrecisionTest, SoftmaxLargePositiveValues) {
    ...
    auto host = zeros({1, 5}, DType::Float32, Device::cpu());
    auto* ptr = host.data<float>();
    ptr[0] = 1000.0f;
    ptr[1] = 1001.0f;
    ptr[2] = 999.0f;
    ptr[3] = 1000.0f;
    ptr[4] = 998.0f;
    auto data = host.to(device);

    Variable x(data, false);
    auto result = tenzor::softmax(x, 1);
    auto result_data = result.tensor().cpu();
...
TEST_P(NumericalPrecisionTest, SoftmaxVeryLargeValues) {
    // Even more extreme values
    auto host = zeros({1, 3}, DType::Float32, Device::cpu());
    auto* ptr = host.data<float>();
    ptr[0] = 1e30f;
...
TEST_P(NumericalPrecisionTest, SoftmaxLargeNegativeValues) {
    auto host = zeros({1, 4}, DType::Float32, Device::cpu());
    auto* ptr = host.data<float>();
    ptr[0] = -1000.0f;
...
TEST_P(NumericalPrecisionTest, SoftmaxUniformInput) {
    // All equal values -> uniform distribution (1/N each)
    int n = 5;
    auto data = full({1, n}, 42.0f, DType::Float32, device);
...
TEST_P(NumericalPrecisionTest, LogSmallPositiveValues) {
    auto host = zeros({4}, DType::Float32, Device::cpu());
    auto* ptr = host.data<float>();
    ptr[0] = 1e-30f;
    ptr[1] = 1e-20f;
    ptr[2] = 1e-10f;
...
TEST_P(NumericalPrecisionTest, LogOfOne) {
...
TEST_P(NumericalPrecisionTest, LogOfLargeValues) {
    auto host = zeros({3}, DType::Float32, Device::cpu());
    auto* ptr = host.data<float>();
    ptr[0] = 1e10f;
    ptr[1] = 1e20f;
    ptr[2] = 1e30f;
...
TEST_P(NumericalPrecisionTest, LogSoftmaxLargeValues) {
...
TEST_P(NumericalPrecisionTest, Float16BasicAccuracy) {
    // Create a Float32 tensor, convert to Float16, convert back, check accuracy
    auto host = zeros({8}, DType::Float32, Device::cpu());
    auto* ptr = host.data<float>();
    ptr[0] = 0.0f;
    ptr[1] = 1.0f;
    ptr[2] = -1.0f;
    ...
    ptr[6] = -100.0f;
    ptr[7] = 0.001f;
    auto data_f32 = host.to(device);

    auto data_f16 = data_f32.to(DType::Float16);
    auto data_roundtrip = data_f16.to(DType::Float32);
    auto rt_cpu = data_roundtrip.cpu();
    auto* rt_ptr = rt_cpu.data<float>();
...
TEST_P(NumericalPrecisionTest, Float16AdditionAccuracy) {
    ...
    auto* a_ptr = a_host.data<float>();
    auto* b_ptr = b_host.data<float>();
    a_ptr[0] = 1.0f; a_ptr[1] = 0.5f; a_ptr[2] = 10.0f; a_ptr[3] = 0.1f;
    b_ptr[0] = 2.0f; b_ptr[1] = 0.25f; b_ptr[2] = 20.0f; b_ptr[3] = 0.2f;
    auto a_f32 = a_host.to(device);
    auto b_f32 = b_host.to(device);

    // Float32 reference
    auto result_f32 = add(a_f32, b_f32);
    auto ref_cpu = result_f32.cpu();
    auto* ref = ref_cpu.data<float>();
    ...
    auto r16_cpu = result_f16_as_f32.cpu();
    auto* r16 = r16_cpu.data<float>();
...
TEST_P(NumericalPrecisionTest, ExpLargePositive) {
    auto host = zeros({2}, DType::Float32, Device::cpu());
    auto* ptr = host.data<float>();
...
TEST_P(NumericalPrecisionTest, ExpLargeNegative) {
    auto host = zeros({2}, DType::Float32, Device::cpu());
    auto* ptr = host.data<float>();
