 ...
 * @brief CPU reduction half-dtype contract — audit items E.1 + E.2.
 *
 * Two CPU reduction kernels previously violated their dtype contracts on
...
class ReductionHalfDtypeTest : public ::testing::Test {
protected:
...
// ---------------------------------------------------------------------------
TEST_F(ReductionHalfDtypeTest, ProdFloat16PreservesDtype) {
...
TEST_F(ReductionHalfDtypeTest, ProdBFloat16PreservesDtype) {
...
TEST_F(ReductionHalfDtypeTest, ProdAlongDimFloat16PreservesDtype) {
    Tensor x_f32({2, 3}, DType::Float32, Device::cpu());
    auto* p = x_f32.data<float>();
    // [[1, 2, 3], [4, 5, 6]]
    for (int i = 0; i < 6; ++i) p[i] = static_cast<float>(i + 1);
