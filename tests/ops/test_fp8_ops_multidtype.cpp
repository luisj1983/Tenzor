/**
 * @file test_fp8_ops_multidtype.cpp
 * @brief Multi-backend tests for FP8 (E4M3 and E5M2) tensor operations.
 *
 * Verifies across all backends that:
...
TEST_P(FP8OpsMultiBackendTest, SameTypePromotion_E4M3) {
    EXPECT_EQ(promote_types(DType::FP8_E4M3, DType::FP8_E4M3), DType::FP8_E4M3);
}

TEST_P(FP8OpsMultiBackendTest, SameTypePromotion_E5M2) {
    EXPECT_EQ(promote_types(DType::FP8_E5M2, DType::FP8_E5M2), DType::FP8_E5M2);
}

TEST_P(FP8OpsMultiBackendTest, MixedFP8Promotion) {
...
TEST_P(FP8OpsMultiBackendTest, FP8Float32Promotion) {
...
TEST_P(FP8OpsMultiBackendTest, FP8Float64Promotion) {
    EXPECT_EQ(promote_types(DType::FP8_E4M3, DType::Float64), DType::Float64);
}

// --- Output DType Tests ---

TEST_P(FP8OpsMultiBackendTest, AddOutputDtype_E4M3) {
...
TEST_P(FP8OpsMultiBackendTest, MulOutputDtype_E4M3) {
    auto a = randn({4, 4}, DType::Float32, device).to(DType::FP8_E4M3);
    auto b = randn({4, 4}, DType::Float32, device).to(DType::FP8_E4M3);
...
TEST_P(FP8OpsMultiBackendTest, DivOutputDtype_E4M3) {
...
TEST_P(FP8OpsMultiBackendTest, MatmulOutputDtype_E4M3) {
...
TEST_P(FP8OpsMultiBackendTest, MixedFP8OutputDtype) {
    auto a = randn({4, 4}, DType::Float32, device).to(DType::FP8_E4M3);
...
TEST_P(FP8OpsMultiBackendTest, FP8Float32MixedOutputDtype) {
    auto a = randn({4, 4}, DType::Float32, device).to(DType::FP8_E4M3);
...
TEST_P(FP8OpsMultiBackendTest, MatmulNumericalCorrectness) {
    ...
    float max_diff = max_diff_t.to(Device::cpu()).data<float>()[0];
...
TEST_P(FP8OpsMultiBackendTest, AddNumericalCorrectness) {
...
TEST_P(FP8OpsMultiBackendTest, QuantizedFloat32Promotion) {
...
TEST_P(FP8OpsMultiBackendTest, QuantizedFloat64Promotion) {
    EXPECT_EQ(promote_types(DType::QInt8, DType::Float64), DType::Float64);
}

TEST_P(FP8OpsMultiBackendTest, QuantizedQuantizedPromotion) {
...
TEST_P(FP8OpsMultiBackendTest, QuantizedIntegerPromotion) {
...
TEST_P(FP8OpsMultiBackendTest, QuantizedSameTypePromotion) {
...
TEST_P(FP8OpsMultiBackendTest, FP8QuantizeOutputDtype) {
...
INSTANTIATE_BACKEND_TESTS(FP8OpsMultiBackendTest);
