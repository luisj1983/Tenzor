...
TEST_P(GradCheckMissingMultiDTypeTest, CholeskyInverse) {
    if (dtype() == DType::Float16 || dtype() == DType::BFloat16) SKIP_WITH_REASON(::tenzor::testing::SkipReason::GradcheckFDPrecision, "Float16/BFloat16 gradcheck precision (fixed absolute FD eps is below both dtypes' representable precision at O(1) magnitude)");
