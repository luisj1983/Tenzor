...
// Cross-backend fixtures (one per plain-TEST suite that was ported).
class LayerNormTest : public ::tenzor::testing::BackendTest {};
class RMSNormTest : public ::tenzor::testing::BackendTest {};
class GroupNormTest : public ::tenzor::testing::BackendTest {};
class NormalizationTest : public ::tenzor::testing::BackendTest {};

// ============================================================================
// LayerNorm Tests
// ============================================================================

TEST_P(LayerNormTest, ConstructorWithAffine) {
    LayerNorm ln({10}, 1e-5, true);
    auto params = ln.parameters();
    EXPECT_EQ(params.size(), 2);  // weight and bias
}

TEST_P(LayerNormTest, ConstructorWithoutAffine) {
    LayerNorm ln({10}, 1e-5, false);
    auto params = ln.parameters();
    EXPECT_EQ(params.size(), 0);  // no learnable parameters
}

TEST_P(LayerNormTest, ForwardNormalization1D) {
    ...
    auto input = Variable(from_data(input_data.data(), {2, 4}, device), false);
...
TEST_P(LayerNormTest, ForwardNormalization2D) {
    ...
    auto input = Variable(from_data(input_data.data(), {1, 2, 2, 2}, device), false);
...
TEST_P(LayerNormTest, ForwardWithAffineTransform) {
    LayerNorm ln({4}, 1e-5, true);
    ln.to(device);

    auto input_data = std::vector<float>{1.0f, 2.0f, 3.0f, 4.0f};
    auto input = Variable(from_data(input_data.data(), {1, 4}, device), false);
...
TEST_P(LayerNormTest, MultipleBatches) {
    ...
    auto input = Variable(from_data(input_data.data(), {3, 3}, device), false);
...
TEST_P(LayerNormTest, BackwardGradientFlow) {
    LayerNorm ln({4}, 1e-5, true);
    ln.to(device);
    ln.train();

    auto input_data = std::vector<float>{1.0f, 2.0f, 3.0f, 4.0f};
    auto input = Variable(from_data(input_data.data(), {1, 4}, device), true);
...
TEST_P(LayerNormTest, NumericalGradientCheck) {
    float eps = 1e-4;
    ...
    for (int out_idx = 0; out_idx < 3; out_idx++) {
        ...
        LayerNorm ln({3}, 1e-5, false);
        ln.to(device);
        auto input = Variable(from_data(input_data.data(), {1, 3}, device), true);
        auto output = ln(input);
        ...
        auto grad_output = from_data(grad_output_data.data(), {1, 3}, device);
        output.backward(grad_output);

        ASSERT_TRUE(input.has_grad()) << "Input should have gradient after backward";
        auto analytical_grad_cpu = input.grad().value().cpu();
        auto analytical_grad = analytical_grad_cpu.data<float>();
        ...
        for (int in_idx = 0; in_idx < 3; in_idx++) {
            ...
            auto input_plus_host = from_data(input_data.data(), {1, 3});
            auto input_plus_data = input_plus_host.data<float>();
            input_plus_data[in_idx] += eps;
            auto input_plus = input_plus_host.to(device);

            auto input_minus_host = from_data(input_data.data(), {1, 3});
            auto input_minus_data = input_minus_host.data<float>();
            input_minus_data[in_idx] -= eps;
            auto input_minus = input_minus_host.to(device);

            auto output_plus = ln_plus(Variable(input_plus, false)).tensor().cpu();
            auto output_minus = ln_minus(Variable(input_minus, false)).tensor().cpu();
...
TEST_P(LayerNormTest, ParameterGradients) {
    LayerNorm ln({4}, 1e-5, true);
    ln.to(device);
    ln.train();
    ...
    for (auto& [name, param] : params) {
        EXPECT_TRUE(param->has_grad());
        auto grad_cpu = param->grad().value().cpu();
...
TEST_P(RMSNormTest, Constructor) {
    RMSNorm rn(10, 1e-6);
    auto params = rn.parameters();
    EXPECT_EQ(params.size(), 1);  // weight only, no bias
}

TEST_P(RMSNormTest, ForwardNormalization1D) {
    // Test with 2D input [batch, features]
    RMSNorm rn(4, 1e-6);
    rn.to(device);
...
TEST_P(RMSNormTest, ForwardNormalization2D) {
    ...
    auto input = Variable(from_data(input_data.data(), {1, 2, 4}, device), false);
...
TEST_P(RMSNormTest, MultipleBatches) {
    RMSNorm rn(3, 1e-6);
    rn.to(device);
...
TEST_P(RMSNormTest, BackwardGradientFlow) {
    RMSNorm rn(4, 1e-6);
    rn.to(device);
    rn.train();
...
TEST_P(RMSNormTest, NumericalGradientCheck) {
    float eps = 1e-4;
    auto input_data = std::vector<float>{1.0f, 2.0f, 3.0f};
    ...
    for (int out_idx = 0; out_idx < 3; out_idx++) {
        ...
        for (int in_idx = 0; in_idx < 3; in_idx++) {
            ...
            auto output_plus = rn_plus(Variable(input_plus, false)).tensor().cpu();
            auto output_minus = rn_minus(Variable(input_minus, false)).tensor().cpu();
...
TEST_P(RMSNormTest, ParameterGradients) {
    RMSNorm rn(4, 1e-6);
    rn.to(device);
    rn.train();
...
TEST_P(RMSNormTest, EpsilonEffect) {
    ...
    auto input1 = Variable(from_data(input_data.data(), {1, 4}, device), false);
    auto input2 = Variable(from_data(input_data.data(), {1, 4}, device), false);

    auto output1 = rn1(input1);
    auto output2 = rn2(input2);

    // Both should produce valid output (all zeros)
    auto data1_cpu = output1.tensor().cpu();
    auto data2_cpu = output2.tensor().cpu();
    auto data1 = data1_cpu.data<float>();
    auto data2 = data2_cpu.data<float>();
...
TEST_P(RMSNormTest, LargeInput) {
    // Test with realistic transformer-like dimensions
    RMSNorm rn(512, 1e-6);
    rn.to(device);

    auto input = Variable(randn({4, 128, 512}, DType::Float32, device), true);
    auto output = rn(input);
...
TEST_P(RMSNormTest, CompareWithManualRMS) {
...
TEST_P(GroupNormTest, ConstructorWithAffine) {
    GroupNorm gn(2, 4, 1e-5, true);
    auto params = gn.parameters();
    EXPECT_EQ(params.size(), 2);  // weight and bias
}

TEST_P(GroupNormTest, ConstructorWithoutAffine) {
    GroupNorm gn(2, 4, 1e-5, false);
    auto params = gn.parameters();
    EXPECT_EQ(params.size(), 0);
}

TEST_P(GroupNormTest, InvalidGroupsThrows) {
    // num_channels must be divisible by num_groups
    EXPECT_THROW(GroupNorm(3, 8), std::runtime_error);
}

TEST_P(GroupNormTest, ForwardNormalization) {
    ...
    auto input = Variable(from_data(input_data.data(), {1, 4, 2, 2}, device), false);
...
TEST_P(GroupNormTest, ForwardWithAffine) {
    GroupNorm gn(2, 4, 1e-5, true);
    gn.to(device);
...
TEST_P(GroupNormTest, MultipleBatches) {
    GroupNorm gn(2, 4, 1e-5, false);
    gn.to(device);
    ...
    auto input = Variable(from_data(input_data.data(), {2, 4, 2, 2}, device), false);

    auto output = gn(input);
    EXPECT_EQ(output.shape()[0], 2);
}

TEST_P(GroupNormTest, SingleGroup) {
...
TEST_P(GroupNormTest, GroupsEqualChannels) {
...
TEST_P(GroupNormTest, BackwardGradientFlow) {
    GroupNorm gn(2, 4, 1e-5, true);
    gn.to(device);
    gn.train();

    auto input_data = std::vector<float>(16, 1.0f);
    auto input = Variable(from_data(input_data.data(), {1, 4, 2, 2}, device), true);

    auto output = gn(input);

    auto grad_output = ones({1, 4, 2, 2}, DType::Float32, device);
    output.backward(grad_output);

    EXPECT_TRUE(input.has_grad());
}

TEST_P(GroupNormTest, NumericalGradientCheck) {
    float eps = 1e-4;
    auto input_data = std::vector<float>{
        1.0f, 2.0f, 3.0f, 4.0f,
        5.0f, 6.0f, 7.0f, 8.0f,
    ...
    for (int out_idx : {0, 5, 10, 15}) {
        ...
        grad_output_data[out_idx] = 1.0f;
        auto grad_output = from_data(grad_output_data.data(), {1, 4, 2, 2}, device);
        output.backward(grad_output);
        ...
        for (int in_idx : {0, 5, 10, 15}) {
            ...
            auto input_plus_host = from_data(input_data.data(), {1, 4, 2, 2});
            auto input_plus_data = input_plus_host.data<float>();
            input_plus_data[in_idx] += eps;
            auto input_plus = input_plus_host.to(device);

            auto input_minus_host = from_data(input_data.data(), {1, 4, 2, 2});
            auto input_minus_data = input_minus_host.data<float>();
            input_minus_data[in_idx] -= eps;
            auto input_minus = input_minus_host.to(device);

            auto output_plus = gn_plus(Variable(input_plus, false)).tensor().cpu();
            auto output_minus = gn_minus(Variable(input_minus, false)).tensor().cpu();
...
TEST_P(GroupNormTest, ParameterGradients) {
    GroupNorm gn(2, 4, 1e-5, true);
    gn.to(device);
    gn.train();
...
TEST_P(NormalizationTest, TrainEvalMode) {
    LayerNorm ln({4}, 1e-5, true);
    ln.to(device);
...
TEST_P(NormalizationTest, ZeroGrad) {
    LayerNorm ln({4}, 1e-5, true);
    ln.to(device);
    ...
    auto output = ln(input);
    output.backward(ones({1, 4}, DType::Float32, device));
...
TEST_P(NormalizationTest, LayerNormLargeInput) {
    // Test with realistic image-like dimensions
    LayerNorm ln({64, 8, 8}, 1e-5, true);
    ln.to(device);

    auto input = Variable(randn({2, 64, 8, 8}, DType::Float32, device), true);
    auto output = ln(input);
...
TEST_P(NormalizationTest, GroupNormLargeInput) {
    // Test with realistic image-like dimensions
...
TEST_P(NormalizationTest, CompareLayerNormGroupNorm) {
    ...
    auto input1 = Variable(from_data(input_data.data(), {1, 4, 2, 2}, device), false);
    auto input2 = Variable(from_data(input_data.data(), {1, 4, 2, 2}, device), false);

    auto output_ln = ln(input1);
    auto output_gn = gn(input2);

    auto ln_cpu = output_ln.tensor().cpu();
    auto gn_cpu = output_gn.tensor().cpu();
    auto ln_data = ln_cpu.data<float>();
    auto gn_data = gn_cpu.data<float>();
...
TEST_P(NormalizationTest, EpsilonEffect) {
    // Test that eps prevents division by zero
...
INSTANTIATE_BACKEND_TESTS(LayerNormTest);
INSTANTIATE_BACKEND_TESTS(RMSNormTest);
INSTANTIATE_BACKEND_TESTS(GroupNormTest);
