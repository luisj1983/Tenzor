...
class BatchNorm2dTestEnvironment : public ::testing::Environment {
public:
    void SetUp() override {
        tenzor::initialize();
    }
...
static ::testing::Environment* const batchnorm2d_env =
    ::testing::AddGlobalTestEnvironment(new BatchNorm2dTestEnvironment);
...
TEST(BatchNorm2dTest, ForwardShapePreservation) {
    // Test that output shape matches input shape
    BatchNorm2d bn(64);

    auto input = Variable(randn({32, 64, 28, 28}), true);
...
TEST(BatchNorm2dTest, ParameterInitialization) {
...
TEST(BatchNorm2dTest, NoAffineParameters) {
...
TEST(BatchNorm2dTest, TrainingModeNormalization) {
    ...
    auto input = Variable(randn({16, 3, 8, 8}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, InferenceModeUsesRunningStats) {
    ...
    bn.train();
    auto train_input = Variable(randn({32, 3, 8, 8}) * 2.0f + 5.0f, false);
    ...
    auto test_input = Variable(randn({16, 3, 8, 8}) * 0.5f - 3.0f, false);
...
TEST(BatchNorm2dTest, RunningStatisticsUpdate) {
    ...
    bn.train();
    auto train_input = Variable(randn({16, 2, 8, 8}) * 2.0f + 5.0f, false);
    ...
    auto test_input = Variable(randn({8, 2, 8, 8}) * 1.0f, false);
    auto eval_output = bn.forward(test_input);

    // Outputs should be valid (no NaN)
    auto eval_data = eval_output.tensor().data<float>();
...
TEST(BatchNorm2dTest, MomentumEffect) {
    ...
    auto input = Variable(randn({16, 2, 8, 8}) * 5.0f, false);
    ...
    auto data_high = output_high.tensor().data<float>();
    auto data_low = output_low.tensor().data<float>();
...
TEST(BatchNorm2dTest, EpsilonPreventsDivisionByZero) {
    ...
    auto input = Variable(ones({16, 2, 8, 8}) * 1.0f, false);
    ...
    EXPECT_NO_THROW({
        ...
        auto data_small = output_small.tensor().data<float>();
        auto data_large = output_large.tensor().data<float>();
...
TEST(BatchNorm2dTest, DifferentEpsilonValues) {
    ...
    auto input = Variable(randn({8, 3, 4, 4}), false);
...
TEST(BatchNorm2dTest, AffineTransformationApplied) {
    ...
    auto input = Variable(zeros({4, 2, 4, 4}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, AffineScaling) {
    ...
    auto data_no_affine = output_no_affine.tensor().data<float>();
...
TEST(BatchNorm2dTest, BatchSize1) {
    // Test with single sample (edge case)
    BatchNorm2d bn(4, 1e-5, 0.1, false);
    bn.train();

    auto input = Variable(randn({1, 4, 8, 8}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, BatchSize16) {
    BatchNorm2d bn(8, 1e-5, 0.1, false);
    bn.train();

    auto input = Variable(randn({16, 8, 16, 16}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, BatchSize32) {
    BatchNorm2d bn(16, 1e-5, 0.1, false);
    bn.train();

    auto input = Variable(randn({32, 16, 32, 32}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, BatchSize64) {
    BatchNorm2d bn(32, 1e-5, 0.1, false);
    bn.train();

    auto input = Variable(randn({64, 32, 8, 8}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, VariableBatchSizes) {
    ...
    for (auto bs : batch_sizes) {
        auto input = Variable(randn({bs, 4, 8, 8}), false);
        EXPECT_NO_THROW({
            auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, SingleChannel) {
    BatchNorm2d bn(1, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({8, 1, 16, 16}), false);
    auto output = bn.forward(input);

    EXPECT_EQ(output.shape()[1], 1);
}

TEST(BatchNorm2dTest, ThreeChannelsRGB) {
    BatchNorm2d bn(3, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({16, 3, 224, 224}), false);
    auto output = bn.forward(input);

    EXPECT_EQ(output.shape()[1], 3);
}

TEST(BatchNorm2dTest, Channel64) {
    BatchNorm2d bn(64, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({32, 64, 56, 56}), false);
    auto output = bn.forward(input);

    EXPECT_EQ(output.shape()[1], 64);
}

TEST(BatchNorm2dTest, Channel256) {
    BatchNorm2d bn(256, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({16, 256, 28, 28}), false);
    auto output = bn.forward(input);

    EXPECT_EQ(output.shape()[1], 256);
}

TEST(BatchNorm2dTest, ChannelMismatchError) {
    // Test that mismatched channels throw error
    BatchNorm2d bn(32, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({16, 64, 8, 8}), false);  // 64 channels, expecting 32
...
TEST(BatchNorm2dTest, SmallSpatialDimensions) {
    BatchNorm2d bn(16, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({32, 16, 1, 1}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, LargeSpatialDimensions) {
    BatchNorm2d bn(32, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({8, 32, 128, 128}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, NonSquareSpatialDimensions) {
    BatchNorm2d bn(16, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({16, 16, 32, 64}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, BackwardPassGradientFlow) {
    // Test that gradients flow through the layer
    BatchNorm2d bn(4, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({8, 4, 8, 8}), true);
    auto output = bn.forward(input);

    // Create gradient
    auto out_shape = output.shape();
    std::vector<int64_t> shape_vec(out_shape.begin(), out_shape.end());
    auto grad_output = ones(shape_vec);
...
TEST(BatchNorm2dTest, ParameterGradients) {
...
TEST(BatchNorm2dTest, VerySmallEpsilon) {
    // Test with very small epsilon
    BatchNorm2d bn(4, 1e-10, 0.1, true);
    bn.train();

    auto input = Variable(randn({8, 4, 8, 8}), false);
...
TEST(BatchNorm2dTest, ConstantInput) {
    // Test with constant input (zero variance)
    BatchNorm2d bn(3, 1e-5, 0.1, false);
    bn.train();

    auto input = Variable(ones({8, 3, 8, 8}) * 5.0f, false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, ExtremeValues) {
    // Test with extreme input values
    BatchNorm2d bn(2, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({8, 2, 8, 8}) * 1000.0f, false);
...
TEST(BatchNorm2dTest, InvalidInputDimensions) {
    // Test that non-4D input throws error
    BatchNorm2d bn(32, 1e-5, 0.1, true);

    auto input_3d = Variable(randn({32, 32, 32}), false);
    EXPECT_THROW(bn.forward(input_3d), std::runtime_error);

    auto input_2d = Variable(randn({32, 32}), false);
    EXPECT_THROW(bn.forward(input_2d), std::runtime_error);

    auto input_5d = Variable(randn({8, 32, 8, 8, 8}), false);
    EXPECT_THROW(bn.forward(input_5d), std::runtime_error);
}

// ==================== Comparison Tests ====================

TEST(BatchNorm2dTest, IndependentChannelNormalization) {
    // Test that channels are normalized independently
    BatchNorm2d bn(3, 1e-5, 0.1, false);
    bn.train();

    // Create input where each channel has different statistics
    auto input_tensor = randn({16, 3, 8, 8});
...
TEST(BatchNorm2dTest, ConsistentWithManualNormalization) {
    // Compare BatchNorm output with manual normalization
    BatchNorm2d bn(2, 1e-5, 0.1, false);
    bn.train();

    auto input = Variable(randn({4, 2, 8, 8}), false);
    auto output = bn.forward(input);
...
TEST(BatchNorm2dTest, StatisticsAccumulationOverMultipleBatches) {
    ...
    for (int i = 0; i < 5; ++i) {
        auto input = Variable(randn({16, 4, 8, 8}) * 2.0f + static_cast<float>(i), false);
        auto output = bn.forward(input);
    ...
    bn.eval();
    auto test_input = Variable(randn({8, 4, 8, 8}), false);
    EXPECT_NO_THROW({
        auto output = bn.forward(test_input);
...
TEST(BatchNorm2dTest, IntegrationWithOtherLayers) {
    ...
    auto conv_output = Variable(randn({16, 32, 28, 28}), true);
...
TEST(BatchNorm2dTest, MultipleForwardPasses) {
    ...
    bn.eval();
    auto test_input = Variable(randn({8, 8, 16, 16}), false);
    EXPECT_NO_THROW({
        auto output = bn.forward(test_input);
        EXPECT_EQ(output.shape()[0], 8);
    });
...
TEST(BatchNorm2dTest, NumericalStability) {
    // Test with various numerical edge cases
    BatchNorm2d bn(4, 1e-5, 0.1, true);
    bn.train();

    // Test with very small values
    auto small_input = Variable(randn({8, 4, 8, 8}) * 1e-6f, false);
    EXPECT_NO_THROW({
    ...
    auto zero_input = Variable(randn({8, 4, 8, 8}) * 1e-8f, false);
    EXPECT_NO_THROW({
...
TEST(BatchNorm2dTest, LargeScaleTest) {
    // Test with larger, more realistic dimensions
    BatchNorm2d bn(128, 1e-5, 0.1, true);
    bn.train();

    auto input = Variable(randn({64, 128, 32, 32}), true);
