/**
 * @file test_nadam_multidtype.cpp
 * @brief Multi-backend multi-dtype tests for NAdam optimizer
 */
...
class NAdamMultiDTypeTest : public MultiBackendDTypeTest {
protected:
...
TEST_P(NAdamMultiDTypeTest, BasicStepReducesLoss) {
    auto params = make_params();
    optim::NAdam opt(params, 1e-2);
...
TEST_P(NAdamMultiDTypeTest, ConvergesOnQuadratic) {
    auto params = make_params();
    optim::NAdam opt(params, 1e-2);
...
TEST_P(NAdamMultiDTypeTest, LrGetSet) {
    auto params = make_params();
    optim::NAdam opt(params, 2e-3);
...
TEST_P(NAdamMultiDTypeTest, StateDictRoundtrip) {
    auto params = make_params();
    optim::NAdam opt(params, 2e-3);
    ...
    optim::NAdam opt2(params2, 1e-5);
    opt2.load_state_dict(state);
    EXPECT_DOUBLE_EQ(opt2.get_lr(), 2e-3);
}

TEST_P(NAdamMultiDTypeTest, ParamsOnCorrectDevice) {
    auto params = make_params();
    optim::NAdam opt(params, 1e-2);
...
INSTANTIATE_MULTI_BACKEND_DTYPE_TESTS(NAdamMultiDTypeTest);
