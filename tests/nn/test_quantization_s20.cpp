...
class QuantizationS20Test : public ::testing::Test {
protected:
    void SetUp() override {
        tenzor::initialize();
    }
    ...
    static Tensor make_ramp(std::vector<int64_t> shape, float lo, float hi) {
        int64_t n = 1;
...
TEST_F(QuantizationS20Test, HistogramObserverRebinsOnRangeExpansion) {
...
TEST_F(QuantizationS20Test, HistogramObserverStreamingNonStationary) {
...
TEST_F(QuantizationS20Test, QuantizedConv1dRealInt8MatchesFp32Reference) {
    ...
    // quantisation error stays well-bounded.
    Tensor fp_weight({out_c, in_c, k}, DType::Float32, Device::cpu());
    {
        float* w = fp_weight.data<float>();
        for (int64_t i = 0; i < fp_weight.numel(); ++i) {
            float v = std::sin(static_cast<float>(i) * 0.37f) * 0.5f;
            w[static_cast<size_t>(i)] = v;
        }
    }

    Tensor fp_input({batch, in_c, l_in}, DType::Float32, Device::cpu());
    {
        float* x = fp_input.data<float>();
        for (int64_t i = 0; i < fp_input.numel(); ++i) {
    ...
    // quantisation should contribute error).
    ...
    {
        const float* x = fp_input.data<float>();
        const float* w = effective_weight.data<float>();
...
TEST_F(QuantizationS20Test, QuantStubObserveModeIsPassthrough) {
    // Build QuantStub with placeholder qparams (overwritten by calibration).
    Tensor scale  = full({1}, 1.0f, DType::Float32, Device::cpu());
    Tensor zp     = zeros({1}, DType::Int32, Device::cpu());
...
TEST_F(QuantizationS20Test, QuantStubQuantizeModeInjectsNoise) {
    // Set scale=2/255 so INT8 round-trip is lossy but bounded.
    const float scale_v = 2.0f / 255.0f;
    Tensor scale  = full({1}, scale_v, DType::Float32, Device::cpu());
    Tensor zp     = zeros({1}, DType::Int32, Device::cpu());
    QuantizationParams qp(scale, zp, QuantDType::INT8,
                          QuantizationScheme::PerTensorSymmetric);
    QuantStub stub(qp);
    ...
    // round-trip is provably lossy.
    Tensor x({128}, DType::Float32, Device::cpu());
    {
        float* d = x.data<float>();
        for (int64_t i = 0; i < x.numel(); ++i) {
            float t = static_cast<float>(i) / static_cast<float>(x.numel() - 1);
...
TEST_F(QuantizationS20Test, QuantStubSteBackwardPassesGradThrough) {
    ...
    Tensor scale  = full({1}, scale_v, DType::Float32, Device::cpu());
    Tensor zp     = zeros({1}, DType::Int64, Device::cpu());
    QuantizationParams qp(scale, zp, QuantDType::INT8,
                          QuantizationScheme::PerTensorSymmetric);
    QuantStub stub(qp);

    // Stay well within +-127*scale = +-1.27 so all activations are in range
    // and the FakeQuantize clip-STE preserves the full gradient.
    Tensor x({8}, DType::Float32, Device::cpu());
    {
        float* d = x.data<float>();
        for (int64_t i = 0; i < x.numel(); ++i) {
