...
Bottleneck::Bottleneck(int64_t in_channels,
    ...
    // 3x3 convolution with groups (standard or grouped for ResNeXt)
    conv2_ = std::make_shared<nn::Conv2d>(
        width, width, 3, stride, 1, 1, groups, false);
    register_module("conv2", conv2_);
...
ResNet::ResNet(const std::vector<int64_t>& layers,
    ...
    if (use_basic_block_) {
        ...
        layer3_ = make_layer_bottleneck(256, layers[2], 2);
        layer4_ = make_layer_bottleneck(512, layers[3], 2);
    }
...
auto ResNet::make_layer_bottleneck(int64_t out_channels, int64_t num_blocks, int64_t stride)
    -> std::shared_ptr<nn::Sequential> {
