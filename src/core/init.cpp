...
auto initialize() -> void {
    ...
    if (should_skip_adaptivecpp) {
        ...
        if (adaptivecpp_result) {
            auto adaptivecpp_backend_unique = std::move(adaptivecpp_result.value());
            auto* adaptivecpp_backend_ptr = adaptivecpp_backend_unique.get();

            // Check if AdaptiveCpp is actually available
            if (adaptivecpp_backend_ptr->is_available()) {
                loader.register_backend(adaptivecpp_backend_ptr->name(), std::move(adaptivecpp_backend_unique));
                std::cout << "AdaptiveCpp backend registered: " << adaptivecpp_backend_ptr->name() << std::endl;
                std::cout << "Found " << adaptivecpp_backend_ptr->device_count() << " AdaptiveCpp device(s)" << std::endl;

                auto* adaptivecpp_backend = adaptivecpp_backend_ptr;

                // Register AdaptiveCpp operations
                std::cout << "Registering AdaptiveCpp kernels with operation registry" << std::endl;

                // Binary math operations
                registry.register_kernel("add", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("add", inputs, attrs);
                    });

                registry.register_kernel("sub", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("sub", inputs, attrs);
                    });

                registry.register_kernel("mul", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("mul", inputs, attrs);
                    });

                registry.register_kernel("div", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("div", inputs, attrs);
                    });

                registry.register_kernel("matmul", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("matmul", inputs, attrs);
                    });

                registry.register_kernel("dot", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("dot", inputs, attrs);
                    });

                // Unary math operations
                registry.register_kernel("sqrt", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("sqrt", inputs, attrs);
                    });

                registry.register_kernel("neg", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("neg", inputs, attrs);
                    });

                registry.register_kernel("abs", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("abs", inputs, attrs);
                    });

                registry.register_kernel("log", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("log", inputs, attrs);
                    });

                registry.register_kernel("exp", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("exp", inputs, attrs);
                    });

                registry.register_kernel("pow", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("pow", inputs, attrs);
                    });

                // Trigonometric operations
                registry.register_kernel("sin", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("sin", inputs, attrs);
                    });

                registry.register_kernel("cos", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("cos", inputs, attrs);
                    });

                registry.register_kernel("tan", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("tan", inputs, attrs);
                    });

                registry.register_kernel("asin", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("asin", inputs, attrs);
                    });

                registry.register_kernel("acos", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("acos", inputs, attrs);
                    });

                registry.register_kernel("atan", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("atan", inputs, attrs);
                    });

                registry.register_kernel("sinh", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("sinh", inputs, attrs);
                    });

                registry.register_kernel("cosh", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("cosh", inputs, attrs);
                    });

                registry.register_kernel("atan2", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("atan2", inputs, attrs);
                    });

                // Rounding operations
                registry.register_kernel("round", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("round", inputs, attrs);
                    });

                registry.register_kernel("floor", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("floor", inputs, attrs);
                    });

                registry.register_kernel("ceil", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("ceil", inputs, attrs);
                    });

                registry.register_kernel("trunc", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("trunc", inputs, attrs);
                    });

                registry.register_kernel("reciprocal", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                ...
                registry.register_kernel("relu", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("relu", inputs, attrs);
                    });

                registry.register_kernel("relu_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("relu_backward", inputs, attrs);
                    });

                registry.register_kernel("sigmoid", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("sigmoid", inputs, attrs);
                    });

                registry.register_kernel("sigmoid_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("sigmoid_backward", inputs, attrs);
                    });

                registry.register_kernel("tanh", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("tanh", inputs, attrs);
                    });

                registry.register_kernel("tanh_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("tanh_backward", inputs, attrs);
                    });

                registry.register_kernel("gelu", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("gelu", inputs, attrs);
                    });

                registry.register_kernel("gelu_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("gelu_backward", inputs, attrs);
                    });

                registry.register_kernel("softmax", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("softmax", inputs, attrs);
                    });

                registry.register_kernel("softmax_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("softmax_backward", inputs, attrs);
                    });

                registry.register_kernel("log_softmax", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("log_softmax", inputs, attrs);
                    });

                registry.register_kernel("log_softmax_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("log_softmax_backward", inputs, attrs);
                    });

                registry.register_kernel("leaky_relu", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("leaky_relu", inputs, attrs);
                    });

                registry.register_kernel("leaky_relu_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("leaky_relu_backward", inputs, attrs);
                    });

                registry.register_kernel("swish", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("swish", inputs, attrs);
                    });

                registry.register_kernel("swish_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("swish_backward", inputs, attrs);
                    });

                // Reduction operations
                registry.register_kernel("sum", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("sum", inputs, attrs);
                    });

                registry.register_kernel("mean", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("mean", inputs, attrs);
                    });

                registry.register_kernel("max", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("max", inputs, attrs);
                    });

                registry.register_kernel("min", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("min", inputs, attrs);
                    });

                registry.register_kernel("argmax", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("argmax", inputs, attrs);
                    });

                registry.register_kernel("argmin", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("argmin", inputs, attrs);
                    });

                registry.register_kernel("std", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("std", inputs, attrs);
                    });

                registry.register_kernel("var", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("var", inputs, attrs);
                    });

                registry.register_kernel("norm", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("norm", inputs, attrs);
                    });

                registry.register_kernel("prod", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("prod", inputs, attrs);
                    });

                // Transform operations
                registry.register_kernel("reshape", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("reshape", inputs, attrs);
                    });

                registry.register_kernel("transpose", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("transpose", inputs, attrs);
                    });

                registry.register_kernel("permute", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("permute", inputs, attrs);
                    });

                registry.register_kernel("squeeze", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("squeeze", inputs, attrs);
                    });

                registry.register_kernel("unsqueeze", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("unsqueeze", inputs, attrs);
                    });

                registry.register_kernel("contiguous", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("contiguous", inputs, attrs);
                    });

                registry.register_kernel("clone", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("clone", inputs, attrs);
                    });

                registry.register_kernel("cat", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("cat", inputs, attrs);
                    });

                registry.register_kernel("expand", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("expand", inputs, attrs);
                    });

                registry.register_kernel("repeat", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("repeat", inputs, attrs);
                    });

                // Fill/creation operations
                registry.register_kernel("zeros", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("zeros", inputs, attrs);
                    });

                registry.register_kernel("ones", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("ones", inputs, attrs);
                    });

                registry.register_kernel("full", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("full", inputs, attrs);
                    });

                registry.register_kernel("fill", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("fill", inputs, attrs);
                    });

                registry.register_kernel("randn", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("randn", inputs, attrs);
                    });

                registry.register_kernel("rand", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("rand", inputs, attrs);
                    });

                // Utility operations
                registry.register_kernel("clamp", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("clamp", inputs, attrs);
                    });

                registry.register_kernel("clamp_min", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("clamp_min", inputs, attrs);
                    });

                registry.register_kernel("clamp_max", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("clamp_max", inputs, attrs);
                    });

                registry.register_kernel("sign", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("sign", inputs, attrs);
                    });

                registry.register_kernel("where", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("where", inputs, attrs);
                    });

                // Comparison operations
                registry.register_kernel("eq", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("eq", inputs, attrs);
                    });

                registry.register_kernel("ne", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("ne", inputs, attrs);
                    });

                registry.register_kernel("lt", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("lt", inputs, attrs);
                    });

                registry.register_kernel("le", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("le", inputs, attrs);
                    });

                registry.register_kernel("gt", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("gt", inputs, attrs);
                    });

                registry.register_kernel("ge", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("ge", inputs, attrs);
                    });

                // Indexing operations
                registry.register_kernel("index_select", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("index_select", inputs, attrs);
                    });

                registry.register_kernel("gather", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("gather", inputs, attrs);
                    });

                registry.register_kernel("scatter", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("scatter", inputs, attrs);
                    });

                registry.register_kernel("masked_select", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("masked_select", inputs, attrs);
                    });

                registry.register_kernel("masked_fill", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("masked_fill", inputs, attrs);
                    });

                // Pooling operations
                registry.register_kernel("avg_pool2d", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("avg_pool2d", inputs, attrs);
                    });

                registry.register_kernel("max_pool2d", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("max_pool2d", inputs, attrs);
                    });

                registry.register_kernel("adaptive_avg_pool2d", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("adaptive_avg_pool2d", inputs, attrs);
                    });

                registry.register_kernel("adaptive_max_pool2d", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("adaptive_max_pool2d", inputs, attrs);
                    });

                registry.register_kernel("avg_pool2d_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("avg_pool2d_backward", inputs, attrs);
                    });

                registry.register_kernel("max_pool2d_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("max_pool2d_backward", inputs, attrs);
                    });

                // Batch normalization
                registry.register_kernel("batchnorm2d_mean_var", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("batchnorm2d_mean_var", inputs, attrs);
                    });

                registry.register_kernel("batchnorm2d_forward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("batchnorm2d_forward", inputs, attrs);
                    });

                registry.register_kernel("batchnorm2d_forward_affine", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("batchnorm2d_forward_affine", inputs, attrs);
                    });

                registry.register_kernel("batchnorm2d_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("batchnorm2d_backward", inputs, attrs);
                    });

                // Conv2d operations
                registry.register_kernel("conv2d_forward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("conv2d_forward", inputs, attrs);
                    });

                registry.register_kernel("conv2d_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                ...
                registry.register_kernel("embedding_lookup", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("embedding_lookup", inputs, attrs);
                    });

                registry.register_kernel("embedding_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("embedding_backward", inputs, attrs);
                    });

                // Im2col/Col2im
                registry.register_kernel("im2col", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("im2col", inputs, attrs);
                    });

                registry.register_kernel("col2im", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("col2im", inputs, attrs);
                    });

                // Fused operations
                registry.register_kernel("fused_add_relu", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("fused_add_relu", inputs, attrs);
                    });

                registry.register_kernel("fused_gelu", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("fused_gelu", inputs, attrs);
                    });

                registry.register_kernel("fused_layer_norm", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("fused_layer_norm", inputs, attrs);
                    });

                registry.register_kernel("fused_layer_norm_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("fused_layer_norm_backward", inputs, attrs);
                    });

                registry.register_kernel("fused_linear_relu", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("fused_linear_relu", inputs, attrs);
                    });

                registry.register_kernel("fused_batchnorm_relu", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("fused_batchnorm_relu", inputs, attrs);
                    });

                registry.register_kernel("fused_matmul_add", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("fused_matmul_add", inputs, attrs);
                    });

                registry.register_kernel("fused_softmax_cross_entropy", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("fused_softmax_cross_entropy", inputs, attrs);
                    });

                // LSTM operations
                registry.register_kernel("lstm_cell_forward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("lstm_cell_forward", inputs, attrs);
                    });

                registry.register_kernel("lstm_cell_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("lstm_cell_backward", inputs, attrs);
                    });

                // GRU operations
                registry.register_kernel("gru_cell_forward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("gru_cell_forward", inputs, attrs);
                    });

                registry.register_kernel("gru_cell_backward", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("gru_cell_backward", inputs, attrs);
                    });

                // Vision operations
                registry.register_kernel("nms", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("nms", inputs, attrs);
                    });

                registry.register_kernel("roi_align", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("roi_align", inputs, attrs);
                    });

                // Quantization operations
                registry.register_kernel("quantize", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("quantize", inputs, attrs);
                    });

                registry.register_kernel("dequantize", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("dequantize", inputs, attrs);
                    });

                registry.register_kernel("quantized_linear", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("quantized_linear", inputs, attrs);
                    });

                registry.register_kernel("quantized_conv2d", Device::Type::AdaptiveCpp,
                    [adaptivecpp_backend](std::span<const Tensor> inputs, const OpAttributes& attrs) -> std::vector<Tensor> {
                        return adaptivecpp_backend->dispatch("quantized_conv2d", inputs, attrs);
                    });

                std::cout << "AdaptiveCpp operations registered successfully (100+ operations)" << std::endl;
            } else {
                std::cout << "AdaptiveCpp backend loaded but no devices available" << std::endl;
            }
        } else {
            std::cout << "Warning: Failed to load AdaptiveCpp backend: " << adaptivecpp_result.error() << std::endl;
        }
    } else {
        std::cout << "AdaptiveCpp backend not found at: " << adaptivecpp_backend_path << std::endl;
    }
