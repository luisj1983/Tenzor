...
__global__ void avgpool1d_forward_impl(
    ...
    for (int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        int64_t divisor = count_include_pad ? kernel_size : (int64_t)count;
        dev_store_compute(output, idx, divisor > 0 ? sum / Compute(divisor) : Compute(0));
    }
}
...
auto maxpool3d_forward_kernel(const Tensor& input, std::array<int64_t, 3> kernel_size,
                               ...
                               cudaStream_t stream) -> std::pair<Tensor, Tensor> {
    // Q.5: per-axis std::array<int64_t, 3>. kernel_size/stride/padding/dilation
    // now honour asymmetric D/H/W (previously collapsed to a single scalar),
    // matching the CPU reference (src/backends/cpu/kernels/pooling.cpp).
    ...
    if (input.dtype() == DType::Float32) {
        ...
        maxpool3d_forward_impl<float><<<grid, block, 0, stream>>>(
            input.data<float>(), output.data<float>(), indices.data<int64_t>(),
            N, C, D, H, W, D_out, H_out, W_out, kD, kH, kW, sD, sH, sW, pD, pH, pW, dD, dH, dW);
        CUDA_CHECK(cudaGetLastError());
    } else if (input.dtype() == DType::Float64) {
...
__global__ void avgpool3d_forward_impl(
    ...
    for (int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        int64_t divisor = count_include_pad ? (kD * kH * kW) : (int64_t)count;
        dev_store_compute(output, idx, divisor > 0 ? sum / static_cast<Compute>(divisor) : Compute(0));
    }
}
...
//   start(out-1) = in - pool
// The earlier code used an adaptive-style DISJOINT ratio partition (in/out) and
// never used the pool size, so kernel_size was a no-op.
    ...
    -> std::pair<Tensor, Tensor>
{
    ...
    if (input.dtype() == DType::Float32) {
        ...
        fractional_maxpool2d_forward_impl<<<grid, block, 0, stream>>>(
            ...
            samples_ptr, N, C, H, W, out_h, out_w, kernel_h, kernel_w, total);
        CUDA_CHECK(cudaGetLastError());
    } else if (input.dtype() == DType::Float64) {
    ...
    -> std::pair<Tensor, Tensor>
{
    ...
    if (input.dtype() == DType::Float32) {
        ...
        fractional_maxpool3d_forward_impl<<<grid, block, 0, stream>>>(
            input.data<float>(), output.data<float>(), idx_out.data<int64_t>(),
            samples_ptr, N, C, D, H, W, out_d, out_h, out_w, kernel_d, kernel_h, kernel_w, total);
        CUDA_CHECK(cudaGetLastError());
    } else if (input.dtype() == DType::Float64) {
        // Native Float64 path.
