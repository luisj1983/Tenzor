...
__device__ inline float bilinear_interpolate_cuda(const float* data, int64_t height,
...
__global__ void roi_align_forward_kernel(
    ...
    for (int64_t iy = 0; iy < roi_bin_grid_h; ++iy) {
        for (int64_t ix = 0; ix < roi_bin_grid_w; ++ix) {
            ...
            // Bilinear interpolation
            sum += bilinear_interpolate_cuda(channel_features, feat_height,
                                              feat_width, y, x);
        }
    }
...
extern "C" void roi_align_forward_cuda(
...
extern "C" void roi_align_backward_cuda(
    ...
    cudaMemset(grad_features, 0, total_features * sizeof(float));
