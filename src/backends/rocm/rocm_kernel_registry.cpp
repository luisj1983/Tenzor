...
void register_rocm_kernels(BackendDispatchTable& table) {
    ...
    table.register_kernel(OpId::FusedRMSPropStep, [](std::span<const Tensor> inputs, const OpAttributes& attrs) {
        ...
        // momentum_buffer. So momentum_buffer is at index 3 when NOT centered
        // and index 4 when centered (mirrors the CUDA kernel fix — hard-coding
        // index 4 silently drops the momentum buffer for the common
        // centered=false, momentum>0 case).
