...
// Forward declarations for AdaptiveCpp/SYCL kernels
...
/**
 * @brief AdaptiveCpp backend implementation for portable SYCL acceleration.
 *
 * Supports multiple backends through AdaptiveCpp:
 * - CUDA (NVIDIA GPUs)
 * - HIP/ROCm (AMD GPUs)
 * - OpenMP (CPU fallback)
...
public:
    ...
    auto allocate(size_t bytes, int32_t device_id) -> void* override {
        ...
        try {
            ...
            throw std::runtime_error(
                std::string("AdaptiveCpp allocation failed: ") + e.what()
            );
        }
    }
    ...
    auto dispatch(const std::string& op_name,
        ...
        for (const auto& tensor : inputs) {
            if (tensor.device().type != Device::Type::AdaptiveCpp) {
                throw std::runtime_error(
                    "AdaptiveCppBackend: All input tensors must be on AdaptiveCpp device, got: " +
        ...
        try {
            ...
            if (op_name == "add") {
                ...
                return {adaptivecpp::add_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "sub") {
                if (inputs.size() != 2) throw std::invalid_argument("sub requires 2 inputs");
                return {adaptivecpp::sub_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "mul") {
                if (inputs.size() != 2) throw std::invalid_argument("mul requires 2 inputs");
                return {adaptivecpp::mul_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "div") {
                if (inputs.size() != 2) throw std::invalid_argument("div requires 2 inputs");
                return {adaptivecpp::div_kernel(inputs[0], inputs[1], queue)};
            }
            ...
            // Unary operations
                ...
                return {adaptivecpp::sqrt_kernel(inputs[0], queue)};
            }
            else if (op_name == "neg") {
                return {adaptivecpp::neg_kernel(inputs[0], queue)};
            }
            else if (op_name == "abs") {
                return {adaptivecpp::abs_kernel(inputs[0], queue)};
            }
            else if (op_name == "log") {
                return {adaptivecpp::log_kernel(inputs[0], queue)};
            }
            else if (op_name == "exp") {
                return {adaptivecpp::exp_kernel(inputs[0], queue)};
            }
            else if (op_name == "pow") {
                float exponent = attrs.contains("exponent") ? std::stof(attrs.at("exponent")) : 2.0f;
                return {adaptivecpp::pow_kernel(inputs[0], exponent, queue)};
            }
            else if (op_name == "dot") {
                return {adaptivecpp::dot_kernel(inputs[0], inputs[1], queue)};
            }

            // Activation functions
            else if (op_name == "relu") {
                return {adaptivecpp::relu_kernel(inputs[0], queue)};
            }
            else if (op_name == "relu_backward") {
                return {adaptivecpp::relu_backward_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "sigmoid") {
                return {adaptivecpp::sigmoid_kernel(inputs[0], queue)};
            }
            else if (op_name == "sigmoid_backward") {
                return {adaptivecpp::sigmoid_backward_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "tanh") {
                return {adaptivecpp::tanh_kernel(inputs[0], queue)};
            }
            else if (op_name == "tanh_backward") {
                return {adaptivecpp::tanh_backward_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "gelu") {
                return {adaptivecpp::gelu_kernel(inputs[0], queue)};
            }
            else if (op_name == "gelu_backward") {
                return {adaptivecpp::gelu_backward_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "softmax") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                return {adaptivecpp::softmax_kernel(inputs[0], dim, queue)};
            }
            else if (op_name == "softmax_backward") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                return {adaptivecpp::softmax_backward_kernel(inputs[0], inputs[1], dim, queue)};
            }
            else if (op_name == "log_softmax") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                return {adaptivecpp::log_softmax_kernel(inputs[0], dim, queue)};
            }
            else if (op_name == "log_softmax_backward") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                return {adaptivecpp::log_softmax_backward_kernel(inputs[0], inputs[1], dim, queue)};
            }
            else if (op_name == "leaky_relu") {
                float alpha = attrs.contains("alpha") ? std::stof(attrs.at("alpha")) : 0.01f;
                return {adaptivecpp::leaky_relu_kernel(inputs[0], alpha, queue)};
            }
            else if (op_name == "leaky_relu_backward") {
                float alpha = attrs.contains("alpha") ? std::stof(attrs.at("alpha")) : 0.01f;
                return {adaptivecpp::leaky_relu_backward_kernel(inputs[0], inputs[1], alpha, queue)};
            }
            else if (op_name == "swish") {
                return {adaptivecpp::swish_kernel(inputs[0], queue)};
            }
            else if (op_name == "swish_backward") {
                return {adaptivecpp::swish_backward_kernel(inputs[0], inputs[1], queue)};
            }
            ...
            // Reduction operations
                ...
                return {adaptivecpp::sum_kernel(inputs[0], dim, keepdim, queue)};
            }
            else if (op_name == "mean") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                bool keepdim = attrs.contains("keepdim") && attrs.at("keepdim") == "1";
                return {adaptivecpp::mean_kernel(inputs[0], dim, keepdim, queue)};
            }
            else if (op_name == "max") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                bool keepdim = attrs.contains("keepdim") && attrs.at("keepdim") == "1";
                return {adaptivecpp::max_kernel(inputs[0], dim, keepdim, queue)};
            }
            else if (op_name == "min") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                bool keepdim = attrs.contains("keepdim") && attrs.at("keepdim") == "1";
                return {adaptivecpp::min_kernel(inputs[0], dim, keepdim, queue)};
            }

            // Transform operations
            else if (op_name == "reshape") {
                std::vector<int64_t> shape = parse_shape(attrs.at("shape"));
                return {adaptivecpp::reshape_kernel(inputs[0], shape, queue)};
            }
                ...
                return {adaptivecpp::transpose_kernel(inputs[0], dim0, dim1, queue)};
            }
            else if (op_name == "permute") {
                std::vector<int64_t> dims = parse_shape(attrs.at("dims"));
                return {adaptivecpp::permute_kernel(inputs[0], dims, queue)};
            }
            else if (op_name == "squeeze") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                return {adaptivecpp::squeeze_kernel(inputs[0], dim, queue)};
            }
            else if (op_name == "unsqueeze") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : 0;
                return {adaptivecpp::unsqueeze_kernel(inputs[0], dim, queue)};
            }
            else if (op_name == "contiguous") {
                return {adaptivecpp::contiguous_kernel(inputs[0], queue)};
            }
            else if (op_name == "clone") {
                return {adaptivecpp::clone_kernel(inputs[0], queue)};
            }
            ...
            // Fill operations
                ...
                Device device = inputs.empty() ? Device::adaptivecpp(device_id) : inputs[0].device();
                return {adaptivecpp::zeros_kernel(shape, dtype, device, queue)};
            }
            else if (op_name == "ones") {
                std::vector<int64_t> shape = parse_shape(attrs.at("shape"));
                DType dtype = parse_dtype(attrs);
                Device device = inputs.empty() ? Device::adaptivecpp(device_id) : inputs[0].device();
                return {adaptivecpp::ones_kernel(shape, dtype, device, queue)};
            }
                ...
                DType dtype = parse_dtype(attrs);
                Device device = inputs.empty() ? Device::adaptivecpp(device_id) : inputs[0].device();
                return {adaptivecpp::full_kernel(shape, value, dtype, device, queue)};
            }
            else if (op_name == "fill") {
                float value = std::stof(attrs.at("value"));
                return {adaptivecpp::fill_kernel(inputs[0], value, queue)};
            }
            ...
            // Random number generation
                ...
                DType dtype = parse_dtype(attrs);
                Device device = inputs.empty() ? Device::adaptivecpp(device_id) : inputs[0].device();
                return {adaptivecpp::randn_kernel(shape, dtype, device, queue)};
            }
            else if (op_name == "rand") {
                std::vector<int64_t> shape = parse_shape(attrs.at("shape"));
                DType dtype = parse_dtype(attrs);
                Device device = inputs.empty() ? Device::adaptivecpp(device_id) : inputs[0].device();
                return {adaptivecpp::rand_kernel(shape, dtype, device, queue)};
            }

            // Trigonometric operations
            else if (op_name == "sin") {
                return {adaptivecpp::sin_kernel(inputs[0], queue)};
            }
            else if (op_name == "cos") {
                return {adaptivecpp::cos_kernel(inputs[0], queue)};
            }
            else if (op_name == "tan") {
                return {adaptivecpp::tan_kernel(inputs[0], queue)};
            }
            else if (op_name == "asin") {
                return {adaptivecpp::asin_kernel(inputs[0], queue)};
            }
            else if (op_name == "acos") {
                return {adaptivecpp::acos_kernel(inputs[0], queue)};
            }
            else if (op_name == "atan") {
                return {adaptivecpp::atan_kernel(inputs[0], queue)};
            }
            else if (op_name == "sinh") {
                return {adaptivecpp::sinh_kernel(inputs[0], queue)};
            }
            else if (op_name == "cosh") {
                return {adaptivecpp::cosh_kernel(inputs[0], queue)};
            }
            else if (op_name == "atan2") {
                return {adaptivecpp::atan2_kernel(inputs[0], inputs[1], queue)};
            }

            // Rounding operations
            else if (op_name == "round") {
                return {adaptivecpp::round_kernel(inputs[0], queue)};
            }
            else if (op_name == "floor") {
                return {adaptivecpp::floor_kernel(inputs[0], queue)};
            }
            else if (op_name == "ceil") {
                return {adaptivecpp::ceil_kernel(inputs[0], queue)};
            }
            else if (op_name == "trunc") {
                return {adaptivecpp::trunc_kernel(inputs[0], queue)};
            }
            else if (op_name == "reciprocal") {
                return {adaptivecpp::reciprocal_kernel(inputs[0], queue)};
            }
            ...
            // Utility operations
                ...
                return {adaptivecpp::clamp_kernel(inputs[0], min_val, max_val, queue)};
            }
            else if (op_name == "clamp_min") {
                float min_val = attrs.contains("min") ? std::stof(attrs.at("min")) : 0.0f;
                return {adaptivecpp::clamp_min_kernel(inputs[0], min_val, queue)};
            }
            else if (op_name == "clamp_max") {
                float max_val = attrs.contains("max") ? std::stof(attrs.at("max")) : 1.0f;
                return {adaptivecpp::clamp_max_kernel(inputs[0], max_val, queue)};
            }
            else if (op_name == "sign") {
                return {adaptivecpp::sign_kernel(inputs[0], queue)};
            }
            else if (op_name == "where") {
                return {adaptivecpp::where_kernel(inputs[0], inputs[1], inputs[2], queue)};
            }
            else if (op_name == "repeat") {
                auto repeats = parse_shape(attrs.at("repeats"));
                return {adaptivecpp::repeat_kernel(inputs[0], repeats, queue)};
            }

            // Fused operations
            else if (op_name == "fused_add_relu") {
                return {adaptivecpp::fused_add_relu_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "fused_gelu") {
                return {adaptivecpp::fused_gelu_kernel(inputs[0], queue)};
            }
                ...
                auto [output, mean, inv_std] = adaptivecpp::fused_layer_norm_kernel(
                ...
                auto normalized_shape = parse_shape(attrs.at("normalized_shape"));
                auto [grad_input, grad_weight, grad_bias] = adaptivecpp::fused_layer_norm_backward_kernel(
                    inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], normalized_shape, queue);
                return {grad_input, grad_weight, grad_bias};
            }
            else if (op_name == "fused_linear_relu") {
                const Tensor* bias = inputs.size() > 2 ? &inputs[2] : nullptr;
                return {adaptivecpp::fused_linear_relu_kernel(inputs[0], inputs[1], bias, queue)};
            }
            else if (op_name == "fused_batchnorm_relu") {
                float epsilon = attrs.contains("epsilon") ? std::stof(attrs.at("epsilon")) : 1e-5f;
                return {adaptivecpp::fused_batchnorm_relu_kernel(
                    inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], epsilon, queue)};
            }
            else if (op_name == "fused_matmul_add") {
                return {adaptivecpp::fused_matmul_add_kernel(inputs[0], inputs[1], inputs[2], queue)};
            }
            else if (op_name == "fused_softmax_cross_entropy") {
                std::string reduction = attrs.contains("reduction") ? attrs.at("reduction") : "mean";
                return {adaptivecpp::fused_softmax_cross_entropy_kernel(inputs[0], inputs[1], reduction, queue)};
            }
            ...
            // LSTM operations
                ...
                auto [h_out, c_out] = adaptivecpp::lstm_cell_forward_kernel(
                ...
                int64_t batch_size = std::stoll(attrs.at("batch_size"));
                int64_t hidden_size = std::stoll(attrs.at("hidden_size"));
                auto [grad_gates, grad_c_prev] = adaptivecpp::lstm_cell_backward_kernel(
            ...
            // GRU operations
                ...
                int64_t batch_size = std::stoll(attrs.at("batch_size"));
                int64_t hidden_size = std::stoll(attrs.at("hidden_size"));
                return {adaptivecpp::gru_cell_forward_kernel(
                    inputs[0], inputs[1], inputs[2], inputs[3], inputs[4],
                    batch_size, hidden_size, queue)};
            }
                ...
                int64_t batch_size = std::stoll(attrs.at("batch_size"));
                int64_t hidden_size = std::stoll(attrs.at("hidden_size"));
                auto outputs = adaptivecpp::gru_cell_backward_kernel(
            ...
            // Vision operations
                ...
                return {adaptivecpp::nms_kernel(inputs[0], inputs[1], iou_threshold, queue)};
            }
                ...
                return {adaptivecpp::roi_align_kernel(inputs[0], inputs[1], output_height, output_width,
            ...
            // Quantization operations
                ...
                return {adaptivecpp::quantize_kernel(inputs[0], scale, zero_point, queue)};
            }
            else if (op_name == "dequantize") {
                float scale = std::stof(attrs.at("scale"));
                int32_t zero_point = std::stoi(attrs.at("zero_point"));
                return {adaptivecpp::dequantize_kernel(inputs[0], scale, zero_point, queue)};
            }
                ...
                return {adaptivecpp::quantized_linear_kernel(inputs[0], inputs[1], bias,
                ...
                float weight_scale = std::stof(attrs.at("weight_scale"));
                int32_t weight_zero_point = std::stoi(attrs.at("weight_zero_point"));
                return {adaptivecpp::quantized_conv2d_kernel(inputs[0], inputs[1], bias,
            ...
            // Comparison operations
                ...
                return {adaptivecpp::eq_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "ne") {
                return {adaptivecpp::ne_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "lt") {
                return {adaptivecpp::lt_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "le") {
                return {adaptivecpp::le_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "gt") {
                return {adaptivecpp::gt_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "ge") {
                return {adaptivecpp::ge_kernel(inputs[0], inputs[1], queue)};
            }

            // Indexing operations
            else if (op_name == "index_select") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : 0;
                return {adaptivecpp::index_select_kernel(inputs[0], dim, inputs[1], queue)};
            }
            else if (op_name == "gather") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : 0;
                return {adaptivecpp::gather_kernel(inputs[0], dim, inputs[1], queue)};
            }
            else if (op_name == "scatter") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : 0;
                return {adaptivecpp::scatter_kernel(inputs[0], dim, inputs[1], inputs[2], queue)};
            }
            else if (op_name == "masked_select") {
                return {adaptivecpp::masked_select_kernel(inputs[0], inputs[1], queue)};
            }
            else if (op_name == "masked_fill") {
                float value = std::stof(attrs.at("value"));
                return {adaptivecpp::masked_fill_kernel(inputs[0], inputs[1], value, queue)};
            }
            ...
            // Pooling operations
                ...
                return {adaptivecpp::adaptive_max_pool2d_kernel(inputs[0], attrs, queue)};
            }
            else if (op_name == "avg_pool2d_backward") {
                return {adaptivecpp::avg_pool2d_backward_kernel(inputs[0], inputs[1], attrs, queue)};
            }
            else if (op_name == "max_pool2d_backward") {
                return {adaptivecpp::max_pool2d_backward_kernel(inputs[0], inputs[1], attrs, queue)};
            }

            // Batch normalization
            else if (op_name == "batchnorm2d_mean_var") {
                return adaptivecpp::batchnorm2d_mean_var(inputs[0], queue);
            }
            else if (op_name == "batchnorm2d_forward") {
                float epsilon = attrs.contains("epsilon") ? std::stof(attrs.at("epsilon")) : 1e-5f;
                return {adaptivecpp::batchnorm2d_forward(inputs[0], inputs[1], inputs[2], epsilon, queue)};
            }
            else if (op_name == "batchnorm2d_forward_affine") {
                float epsilon = attrs.contains("epsilon") ? std::stof(attrs.at("epsilon")) : 1e-5f;
                return {adaptivecpp::batchnorm2d_forward_affine(inputs[0], inputs[1], inputs[2],
                                                           inputs[3], inputs[4], epsilon, queue)};
            }
            else if (op_name == "batchnorm2d_backward") {
                float epsilon = attrs.contains("epsilon") ? std::stof(attrs.at("epsilon")) : 1e-5f;
                auto [grad_input, grad_gamma, grad_beta] = adaptivecpp::batchnorm2d_backward(
                    inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], epsilon, queue);
            ...
            // Conv2d operations
                ...
                int64_t dilation = attrs.contains("dilation") ? std::stoll(attrs.at("dilation")) : 1;
                int64_t groups = attrs.contains("groups") ? std::stoll(attrs.at("groups")) : 1;
                return {adaptivecpp::conv2d_forward(inputs[0], inputs[1], bias, stride, padding,
                ...
                auto [grad_input, grad_weight, grad_bias] = adaptivecpp::conv2d_backward(
            ...
            // Embedding operations
                ...
                return {adaptivecpp::embedding_lookup_kernel(inputs[0], inputs[1], padding_idx, queue)};
            }
                ...
                return {adaptivecpp::embedding_backward_kernel(inputs[0], inputs[1], vocab_size, embedding_dim, queue)};
            }

            // Statistical operations
            else if (op_name == "std") {
                return {adaptivecpp::std_kernel(inputs[0], attrs, queue)};
            }
            else if (op_name == "var") {
                return {adaptivecpp::var_kernel(inputs[0], attrs, queue)};
            }
            else if (op_name == "norm") {
                return {adaptivecpp::norm_kernel(inputs[0], attrs, queue)};
            }
            else if (op_name == "prod") {
                return {adaptivecpp::prod_kernel(inputs[0], attrs, queue)};
            }
            ...
            // Argmax/Argmin
                ...
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                bool keepdim = attrs.contains("keepdim") && attrs.at("keepdim") == "1";
                return {adaptivecpp::argmax_kernel(inputs[0], dim, keepdim, queue)};
            }
            else if (op_name == "argmin") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : -1;
                bool keepdim = attrs.contains("keepdim") && attrs.at("keepdim") == "1";
                return {adaptivecpp::argmin_kernel(inputs[0], dim, keepdim, queue)};
            }

            // Cat operation
            else if (op_name == "cat") {
                int64_t dim = attrs.contains("dim") ? std::stoll(attrs.at("dim")) : 0;
                return {adaptivecpp::cat_kernel(inputs, dim, queue)};
            }

            // Im2col/Col2im
            else if (op_name == "im2col") {
                return {adaptivecpp::im2col_kernel(inputs[0], attrs, queue)};
            }
            else if (op_name == "col2im") {
                return {adaptivecpp::col2im_kernel(inputs[0], attrs, queue)};
            }

            // Expand
            else if (op_name == "expand") {
                return {adaptivecpp::expand_kernel(inputs[0], attrs, queue)};
            }

            else {
                throw std::runtime_error("AdaptiveCppBackend: Unknown operation '" + op_name + "'");
            }
        }
        catch (const sycl::exception& e) {
            throw std::runtime_error(
                "AdaptiveCppBackend: Operation '" + op_name + "' failed with SYCL error: " + e.what()
            );
        }
    }
...
extern "C" {
    auto create_backend() -> std::unique_ptr<Backend> {
        return std::make_unique<AdaptiveCppBackend>();
    }
}
