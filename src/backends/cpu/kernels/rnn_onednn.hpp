...
inline bool lstm_multilayer_forward_onednn(
    ...
    // Old fused multi-layer code (disabled - sequential is faster)
#if 0
    try {
        auto& engine = get_engine();
        auto& stream = get_stream();
        auto& cached = get_lstm_multilayer_cached();

        bool has_bias = !bias_list.empty() && bias_list[0] != nullptr;

        // Fast inference path: only check dimensions (weights assumed stable)
        bool need_rebuild = !cached ||
                           !cached->matches_dims(num_layers, seq_len, batch, input_size,
                                                 hidden_size, has_bias);

        if (need_rebuild) {
            cached = std::make_shared<LSTMMultiLayerCachedPrimitive>();
            ...
            if (!can_fuse_all) {
                cached->layer0_cache = std::make_shared<LSTMCachedPrimitive>();
            }
            ...
            if (fused_layers > 0) {
                ...
                // Input to fused layers is hidden_size (output from layer 0 or input if can_fuse_all)
                ...
                dnnl::memory::dims weights_layer_dims = {fused_layers, 1, fused_input_size, 4, hidden_size};
                dnnl::memory::dims weights_iter_dims = {fused_layers, 1, hidden_size, 4, hidden_size};
                ...
                auto src_iter_md = dnnl::memory::desc(src_iter_dims, dnnl::memory::data_type::f32,
                                                       dnnl::memory::format_tag::ldnc);
                auto src_iter_c_md = dnnl::memory::desc(src_iter_dims, dnnl::memory::data_type::f32,
                                                         dnnl::memory::format_tag::ldnc);
                // Use format_tag::any for weights to enable optimized brgemm implementation
                auto weights_layer_md = dnnl::memory::desc(weights_layer_dims, dnnl::memory::data_type::f32,
                                                            dnnl::memory::format_tag::any);
                auto weights_iter_md = dnnl::memory::desc(weights_iter_dims, dnnl::memory::data_type::f32,
                                                           dnnl::memory::format_tag::any);
                ...
                auto dst_iter_md = dnnl::memory::desc(dst_iter_dims, dnnl::memory::data_type::f32,
                                                       dnnl::memory::format_tag::ldnc);
                auto dst_iter_c_md = dnnl::memory::desc(dst_iter_dims, dnnl::memory::data_type::f32,
                                                         dnnl::memory::format_tag::ldnc);
                ...
                cached->src_iter_c_mem = dnnl::memory(src_iter_c_md, engine);
                cached->dst_layer_mem = dnnl::memory(dst_layer_md, engine);
                cached->dst_iter_mem = dnnl::memory(dst_iter_md, engine);
                cached->dst_iter_c_mem = dnnl::memory(dst_iter_c_md, engine);

                // Pack weights for all fused layers
                // Layout: ldigo = (layers, directions, input, gates, output)
                int64_t start_layer = can_fuse_all ? 0 : 1;
                size_t w_layer_size = fused_input_size * 4 * hidden_size;
                size_t w_iter_size = hidden_size * 4 * hidden_size;
                ...
                for (int64_t l = 0; l < fused_layers; ++l) {
                    ...
                    // Pack bias
                    if (has_bias) {
                        std::memcpy(bias_packed.data() + l * 4 * hidden_size,
                                   bias_list[src_layer], 4 * hidden_size * sizeof(float));
                    } else {
                        std::memset(bias_packed.data() + l * 4 * hidden_size,
                                   0, 4 * hidden_size * sizeof(float));
                    }
                }
        ...
        std::vector<float> intermediate_c;

        const float* fused_input = input;
        const float* fused_h0 = h0;
        const float* fused_c0 = c0;
        ...
        if (!can_fuse_all) {
            ...
            intermediate_c.resize(batch * hidden_size);

            // Run single-layer LSTM for layer 0
            bool layer0_ok = lstm_forward_onednn(
                input,
        ...
        if (cached->has_fused_layers && fused_layers > 0) {
            ...
            cached->src_iter_c_mem.set_data_handle(const_cast<float*>(fused_c0));
            cached->dst_layer_mem.set_data_handle(output);
            ...
            if (can_fuse_all) {
                ...
                // States for layers 1+ go after layer 0's states
                cached->dst_iter_mem.set_data_handle(h_n + batch * hidden_size);
                cached->dst_iter_c_mem.set_data_handle(c_n + batch * hidden_size);
            }
...
#endif  // Old fused multi-layer code
}
