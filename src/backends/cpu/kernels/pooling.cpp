                ...
                for (int64_t ow = 0; ow < W_out; ++ow) {
                    ...
                    for (int64_t kh = 0; kh < kernel_h; ++kh) {
                        for (int64_t kw = 0; kw < kernel_w; ++kw) {
                            int64_t h = h_start + kh * dilation_h;
                            int64_t w = w_start + kw * dilation_w;
...
auto maxpool2d_forward_kernel(const Tensor& input_orig,
    ...
    auto dispatch = [&](auto* dummy) {
        ...
        maxpool2d_forward_impl<T>(input.data<T>(), output.data<T>(), idx_data,
                                  N, C, H, W, H_out, W_out,
        ...
        for (int64_t c = 0; c < C; ++c) {
            for (int64_t oh = 0; oh < H_out; ++oh) {
                for (int64_t ow = 0; ow < W_out; ++ow) {
                    ...
                    int64_t w = max_idx % W;
                    int64_t in_idx = ((n * C + c) * H + h) * W + w;
                    // Accumulate in double for Float64, float otherwise (half types
                    // widen to float); never round Float64 through float32.
                    using Compute = std::conditional_t<std::is_same_v<T, double>, double, float>;
                    Compute val = static_cast<Compute>(grad_in_data[in_idx]) + static_cast<Compute>(grad_out_data[out_idx]);
...
auto avgpool2d_forward_kernel(const Tensor& input_orig,
    ...
    auto dispatch = [&](auto* dummy) {
        using T = std::remove_pointer_t<decltype(dummy)>;
        avgpool2d_forward_impl<T>(input.data<T>(), output.data<T>(),
                                  N, C, H, W, H_out, W_out,
                                  kernel_size[0], kernel_size[1],
                                  stride[0],      stride[1],
                                  padding[0],     padding[1],
...
auto avgpool2d_backward_kernel(const Tensor& grad_output_orig,
    ...
    auto dispatch = [&](auto* dummy) {
        using T = std::remove_pointer_t<decltype(dummy)>;
        avgpool2d_backward_impl<T>(grad_output.data<T>(), grad_input.data<T>(),
                                   N, C, H, W, H_out, W_out,
                                   kernel_size[0], kernel_size[1],
                                   stride[0],      stride[1],
                                   padding[0],     padding[1],
        ...
        for (int64_t c = 0; c < C; ++c) {
            for (int64_t oh = 0; oh < output_h; ++oh) {
                for (int64_t ow = 0; ow < output_w; ++ow) {
                    ...
                    if (count <= 0) continue;
                    Compute grad_val = static_cast<Compute>(grad_out_data[((n * C + c) * output_h + oh) * output_w + ow]) / static_cast<Compute>(count);
    ...
    for (int64_t n = 0; n < N; ++n) {
        for (int64_t c = 0; c < C; ++c) {
            for (int64_t ol = 0; ol < L_out; ++ol) {
                ...
                int64_t divisor = count_include_pad ? kernel_size : valid_count;
                if (divisor <= 0) continue;
                Compute grad_val = static_cast<Compute>(grad_out_data[(n * C + c) * L_out + ol])
                                 / static_cast<Compute>(divisor);
    ...
    for (int64_t n = 0; n < N; ++n) {
        for (int64_t c = 0; c < C; ++c) {
            for (int64_t ol = 0; ol < L_out; ++ol) {
                ...
                int64_t count = l_end - l_start;
                if (count <= 0) continue;
                Compute grad_val = static_cast<Compute>(grad_out_data[(n * C + c) * L_out + ol]) / static_cast<Compute>(count);
    ...
    for (int64_t n = 0; n < N; ++n) {
        for (int64_t c = 0; c < C; ++c) {
            for (int64_t od = 0; od < D_out; ++od) {
                for (int64_t oh = 0; oh < H_out; ++oh) {
                    for (int64_t ow = 0; ow < W_out; ++ow) {
                        int64_t d_start = od * stride_d - padding_d;
                        int64_t h_start = oh * stride_h - padding_h;
                        int64_t w_start = ow * stride_w - padding_w;
                        ...
                        for (int64_t kd = 0; kd < kernel_d; ++kd) {
                            for (int64_t kh = 0; kh < kernel_h; ++kh) {
                                for (int64_t kw = 0; kw < kernel_w; ++kw) {
                                    int64_t d = d_start + kd * dilation_d;
                                    int64_t h = h_start + kh * dilation_h;
                                    int64_t w = w_start + kw * dilation_w;
...
auto maxpool3d_forward_kernel(const Tensor& input_orig,
    ...
    auto dispatch = [&](auto* dummy) {
        using T = std::remove_pointer_t<decltype(dummy)>;
        maxpool3d_forward_impl<T>(input.data<T>(), output.data<T>(), idx_data,
...
auto avgpool3d_forward_kernel(const Tensor& input_orig,
    ...
    auto dispatch = [&](auto* dummy) {
        using T = std::remove_pointer_t<decltype(dummy)>;
        avgpool3d_forward_impl<T>(input.data<T>(), output.data<T>(),
                                  N, C, D, H, W, D_out, H_out, W_out,
                                  kernel_size[0], kernel_size[1], kernel_size[2],
                                  stride[0],      stride[1],      stride[2],
                                  padding[0],     padding[1],     padding[2],
...
auto avgpool3d_backward_kernel(const Tensor& grad_output_orig,
    ...
    auto dispatch = [&](auto* dummy) {
        using T = std::remove_pointer_t<decltype(dummy)>;
        avgpool3d_backward_impl<T>(grad_output.data<T>(), grad_input.data<T>(),
                                   N, C, D, H, W, D_out, H_out, W_out,
                                   kernel_size[0], kernel_size[1], kernel_size[2],
                                   stride[0],      stride[1],      stride[2],
                                   padding[0],     padding[1],     padding[2],
    ...
    for (int64_t n = 0; n < N; ++n) {
        for (int64_t c = 0; c < C; ++c) {
            ...
            for (int64_t i = 0; i < out_spatial; ++i) {
                ...
                Compute acc = static_cast<Compute>(grad_in_data[base_in + max_idx]);
                acc += static_cast<Compute>(grad_out_data[base_out + i]);
